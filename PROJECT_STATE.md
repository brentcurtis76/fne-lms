...
- **Fase en curso: Z0B — Spikes técnicos (plan Zoom §15), rama `feat/zoom-spike`, PR [#25](https://github.com/brentcurtis76/fne-lms/pull/25). Z0B-1 + Z0B-2 entregados; ronda de remediación **Z0B-2r1** aplicada sobre el veredicto R1 de Sol (REQUEST CHANGES, 7 MAJOR / 2 MINOR — los nueve triados VALID). A la espera de re-review independiente (Sol §0.2).** Z0B-1 (spikes sin credenciales) sellado tras 6 rondas de remediación del sanitizador; Z0B-2 (spikes con Zoom en vivo) cerró el alcance; Z0B-2r1 corrige los nueve hallazgos.
  - **Captura de webhooks: COMPLETA.** Los **7 eventos** del alcance llegaron verificados por firma a través de **14 requests** de Zoom (incluye 2 pares de reintento a 304 s); la biblioteca de fixtures tiene **7 archivos** en `__tests__/lib/zoom/fixtures/webhooks/`, todos sintéticos y auto-consistentes. La URL de suscripción en Marketplace se validó en la propia sesión Z0B-2; la redacción anterior ("bloqueado en un paso humano fuera de la sesión") era una instantánea de mitad de chunk que nunca se reconcilió
  - **Gate G2: FAIL DEFINITIVO**, no provisional. Con los scopes completos se sondaron **13 endpoints**: 9 respondieron, 6 filas de participante inspeccionadas, **cero campos de consentimiento en ninguna respuesta**; los 4 restantes quedaron bloqueados por *entitlement* ("Business or higher"), que es un negativo más fuerte que un bloqueo por scope. Coherente con G1 FAIL (cuenta Pro) — ver `docs/planning/zoom-spike-results.md` §9
  - **DoD parcialmente enmendado por el owner:** los *tamaños reales* de MP4/M4A y la tabla de costo/throughput que depende de ellos se posponen al soak de Z4 por **enmienda aprobada por Brent (2026-07-29)** — no por aceptación del PM. Texto íntegro en `zoom-spike-results.md` §8.1.1; item abierto 21 es el punto de control
- Date: 2026-07-29
...
- Commit SHA (Z0B): `feat/zoom-spike` — base `2786fa8`, **39 commits** en total, HEAD `4b32b47`. Desglose que coincide con el ledger de §0 y con el dossier del PM: **Z0B-1 `2c1f72f`…`41281b9` (19, incluye 1 de docs del PM)** · **Z0B-2 `ace0898`…`7cb0b4b` (12)** · **2 de docs del PM (`a402c3b`, `aabfeec`)** · **Z0B-2r1 `0c8b498`…`4b32b47` (5)**. 19+12+2+5 = 39. (La cifra anterior citaba `0061dbe` como cierre de Z0B-1, que no es el SHA sellado)
...
## Test status
Al cierre de **Z0B-2r1** (`feat/zoom-spike` @ `4b32b47`, local, macOS): **typecheck limpio · lint limpio (zero warnings) · 3351/3351 en 222 archivos · build OK** (`/api/meet/diag-signature` y `/meet/diag` presentes en el output). Delta de Z0B-2r1 sobre `aabfeec` (3112/217): **+239 tests / +5 archivos**.
