...
- **Fase en curso: Z0B — Spikes técnicos (plan Zoom §15), rama `feat/zoom-spike`, PR [#25](https://github.com/brentcurtis76/fne-lms/pull/25). Z0B-2 entregado ⇒ Z0B COMPLETA a la espera de review independiente (Sol §0.2).** Z0B-1 (spikes sin credenciales) sellado tras 6 rondas de remediación del sanitizador; Z0B-2 (spikes con Zoom en vivo) cierra la fase. **Dos entregables quedan parciales y bloqueados en pasos humanos fuera de la sesión**, declarados en `docs/planning/reviews/fase-2-review-request.md`: la captura de payloads reales de webhook (requiere validar la URL de suscripción en Marketplace) y el veredicto **G2, que es FAIL provisional** (10 de 13 sondas bloqueadas por scopes S2S faltantes)
- Date: 2026-07-29
...
- Commit SHA (Z0B): `feat/zoom-spike` — base `2786fa8`; Z0B-1 hasta `0061dbe` (19 commits, sellado), Z0B-2 encima
...
- **Z0B — Spikes técnicos Zoom (2026-07-29, `feat/zoom-spike`, plan Zoom §15, NO es fase del itinerario):** sin cambios de schema, sin migraciones, sin flags nuevos, sin deployments. Todo el contenido de reunión es sintético; cada reunión de prueba se llama `PRUEBA SPIKE — no unirse`.
  - Superficie de producto (única): `pages/meet/diag.tsx` gana el bloque **"Prueba de conexión"** — número + clave de reunión a mano, join Component View, tiempo hasta entrar medido solo y volcado al JSON copiable. Sin `NEXT_PUBLIC_ZOOM_SDK_CLIENT_ID` el bloque renderiza el placeholder y **el resto del diagnóstico sigue funcionando** (aserción explícita en tests: un despliegue sin credenciales no puede dejar a una consultora en terreno sin instrumento)
  - Ruta nueva: `pages/api/meet/diag-signature.ts` — firma el JWT del Meeting SDK. `role:0` **fijado en el servidor** (§5: el rol que manda el cliente se ignora), 404 cuando falta el par de env (función ausente ⇒ ruta ausente), exige sesión. No toca ZAK. La reemplaza el endpoint de join real en Z2/Z3
  - Instrumentación desechable: `scripts/spikes/zoom/**` (cliente S2S, PoC de customerKey, round trip de grabación, control start/stop, sonda G2, cliente SigV4 S3 propio sin dependencias) y `scripts/spikes/webhook/**` (receptor CRC + generador de fixtures). Interlock de seguridad `assertSpikeMeeting()` antes de **cada** llamada destructiva
...
## Test status
Al cierre de **Z0B-2** (`feat/zoom-spike`, local, macOS): **typecheck limpio · lint limpio (zero warnings) · 3094/3094 en 217 archivos · build OK**. Delta de Z0B-2 sobre la línea base de Z0B-1 (3072/215): **+22 tests / +2 archivos** — 14 vectores HMAC/CRC de webhook (`__tests__/lib/zoom/webhook-signature-vectors.test.ts`, incluye el requisito §17 "re-serialized-body-must-fail") y 8 del bloque de prueba de conexión de `/meet/diag` (`__tests__/pages/meet-diag-join-section.test.ts`). `test:db` no aplica (cero migraciones); `e2e` no corrido localmente (los specs de `/meet` son stubs `.skip()` hasta Z1c). **La suite de fixtures de webhook está vacía y lo dice en voz alta**: el test imprime el conteo de fixtures en CI para que una biblioteca vacía no pase por verde silencioso.
