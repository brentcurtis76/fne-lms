...
- **Fase en curso: Z1b — Esquema seguro + jobs + webhook + snapshot de horas (plan Zoom §15), rama `feat/zoom-core`, PR [#26](https://github.com/brentcurtis76/fne-lms/pull/26) (borrador, no marcar *ready* hasta cerrar la fase).**
  ...
  - **Z1b-2 — en curso**: capa cliente `lib/zoom/*` (proveedor de token S2S, cliente REST, firmante JWT del SDK, primitivas de verificación de webhooks, seam `ZoomApi` + fake). El chunk empieza absorbiendo `origin/main` en la rama, que es donde entra todo Z0B (sanitizador, fixtures, vectores de firma, `/meet/diag`)
  - **Pendiente en la fase: Z1b-3** — runtime de jobs, ticker cron, ruta de webhook, `host_sync`/reconcile, y repuntar la suscripción de Marketplace desde el túnel muerto a la ruta real
- Date: 2026-07-30
