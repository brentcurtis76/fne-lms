-- =============================================================================
-- ACTIVITY FEED SYSTEM - SUPABASE SQL EDITOR SETUP (CORRECTED)
...
-- Step 1: Create custom enums for activity feed system
...
-- Step 2: Create core activity feed tables
...
-- Step 5: Create RLS Policies (using correct profiles table)

-- Activity Feed Policies
CREATE POLICY "Users can view public activities in their workspaces" ON activity_feed
    FOR SELECT USING (
        is_public = true OR 
        user_id = auth.uid() OR
        EXISTS (
...
CREATE POLICY "Users can view aggregations for their workspaces" ON activity_aggregations
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE id = auth.uid() 
            AND role IN ('admin', 'consultant', 'lider_comunidad')
        )
    );

CREATE POLICY "Admins can manage aggregations" ON activity_aggregations
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE id = auth.uid() 
...
CREATE OR REPLACE FUNCTION get_activity_stats(p_workspace_id UUID)
RETURNS JSONB AS $$
DECLARE
    stats JSONB := '{}';
    total_count INTEGER;
...
-- Step 7: Enable Realtime for activity_feed table
ALTER PUBLICATION supabase_realtime ADD TABLE activity_feed;

-- Step 8: Create some sample data for testing (optional)
INSERT INTO activity_feed (
    workspace_id, 
    user_id, 
    activity_type, 
    ...
    (
        ...
        'workspace_created', 
        'workspace', 
        'Espacio colaborativo creado', 
        'El sistema de feed de actividades ha sido configurado exitosamente',
        3
    ),
    (
        gen_random_uuid(), 
        ...
        'Bienvenido al espacio colaborativo de FNE',
        2
    );
...
-- Success message
SELECT 'Activity Feed System setup completed successfully! 🎉' as setup_status;