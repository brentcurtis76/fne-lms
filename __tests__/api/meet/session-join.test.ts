...
function buildClient(results: TableResults) {
  return {
    from: vi.fn((table: string) => {
      ...
      return chainable(results[table] ?? { data: null, error: null });
    }),
  };
}
...
function arrange(scenario: Scenario = {}) {
  mockCreateServiceRoleClient.mockImplementation(() =>
    buildClient({
      consultor_sessions: {
        data: 'session' in scenario ? scenario.session : sessionRow,
        error: null,
      },
      session_facilitators: {
        data: scenario.isFacilitator ? { id: 'sf-1' } : null,
        error: null,
      },
      session_attendees: {
        data: scenario.isExpectedAttendee ? { id: 'sa-1' } : null,
        error: null,
      },
