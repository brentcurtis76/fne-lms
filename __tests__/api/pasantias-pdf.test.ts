...
/**
 * Only the two I/O functions are replaced. `StorageObjectExistsError` is the
 * real class, because the route branches on it with `instanceof` — a stand-in
 * defined here would pass the test while the production check silently fell
 * through to the error branch.
 */
vi.mock('@/lib/propuestas/storage', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@/lib/propuestas/storage')>()),
...
import { StorageObjectExistsError } from '@/lib/propuestas/storage';
...
describe.each(DOCUMENTS)('/api/pasantias/$label', (doc) => {
  ...
  describe('cache miss, in production', () => {
    ...
    /**
     * The options object is the whole finding, so it is asserted on its own
     * rather than only as part of the call above: `upsert: true` here would let
     * a generated document replace the owner's published one, and every other
     * assertion in this file would still pass.
     */
    it('asks for a create-only upload, so a fill can never overwrite the override', async () => {
      await get(doc.handler);

      const options = mockUploadFile.mock.calls[0][3];
      expect(options).toEqual({ upsert: false });
      expect(options.upsert).not.toBe(true);
    });

    /**
     * The race Sol's [B1] describes: the object exists — because the owner
     * uploaded the designed brochure, or another cold request finished first —
     * even though our read said miss. Create-only turns that into a refused
     * write instead of an overwrite, and a refused write is not an incident.
     */
    it('leaves an existing object alone and still serves 200 with the generated bytes', async () => {
      mockUploadFile.mockRejectedValue(new StorageObjectExistsError(doc.cachePath));
      ...
      expect(console.info).toHaveBeenCalledWith(
        expect.stringContaining(`left the existing object at ${doc.cachePath} alone`)
      );
    });
    ...
    it('still serves 200 when the upload fails — a broken bucket is not a broken download', async () => {
      ...
      // A real failure is not the benign conflict, and must not be logged as one.
  ...
  /**
   * Sol's [B1], end to end, against a bucket that holds state instead of two
   * mocks that happen to agree with each other. Both orderings in the finding
   * are here: the object exists throughout a failed *read*, and the object
   * appears during a slow *render* after a genuine miss. In both, the question
   * is the same — what is at the path afterwards.
   */
  describe('D-05 override survives a generated fill', () => {
    const DESIGNED_PDF = Buffer.from('%PDF-1.7 owner-designed override\n%%EOF\n', 'utf8');

    /**
     * One slot, and the two halves of the storage contract that matter here:
     * a download rejects when the slot is empty, and a create-only upload
     * rejects when it is full — leaving what is in it untouched.
     */
    function bucketWith(initial: Buffer | null) {
      const slot = { object: initial };

      mockDownloadFile.mockImplementation(async () => {
        if (!slot.object) throw new Error('Object not found');
        return slot.object;
      });
      mockUploadFile.mockImplementation(
        async (p: string, body: Buffer, _contentType: string, options?: { upsert?: boolean }) => {
          if (slot.object && options?.upsert === false) throw new StorageObjectExistsError(p);
          slot.object = body;
          return p;
        }
      );
    ...
    it('keeps the published file when the read that missed it was only transient', async () => {
      const slot = bucketWith(DESIGNED_PDF);
      // The object is healthy; the read is not.
      mockDownloadFile.mockRejectedValueOnce(new Error('503 upstream timeout'));

      const during = await get(doc.handler);

      expect(during._getStatusCode()).toBe(200);
      expect(during._getBuffer().equals(GENERATED_PDF)).toBe(true);
      expect(mockUploadFile).toHaveBeenCalledTimes(1);
      expect(slot.object).toBe(DESIGNED_PDF);

      // And from the next request on, the designed document is what readers get.
      const after = await get(doc.handler);

      expect(after._getBuffer().equals(DESIGNED_PDF)).toBe(true);
      expect(after._getBuffer().equals(GENERATED_PDF)).toBe(false);
      expect(doc.generator).toHaveBeenCalledTimes(1);
    });

    it('keeps a file published while the generator was still rendering', async () => {
      const slot = bucketWith(null);
      // A genuine miss, a slow render, and the owner publishes in between.
      doc.generator.mockImplementation(async () => {
        slot.object = DESIGNED_PDF;
    ...
    it('does fill an empty path, so the cache still warms up', async () => {
      const slot = bucketWith(null);

      await get(doc.handler);

      expect(slot.object).toBe(GENERATED_PDF);

      const after = await get(doc.handler);

      expect(after._getBuffer().equals(GENERATED_PDF)).toBe(true);
      expect(doc.generator).toHaveBeenCalledTimes(1);
    });
  });
