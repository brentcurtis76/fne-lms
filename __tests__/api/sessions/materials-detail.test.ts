...
import handler from '../../../pages/api/sessions/[id]/materials/[mid]';
...
describe('/api/sessions/[id]/materials/[mid]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return 405 for non-DELETE requests', async () => {
    const { req, res } = createMocks({
      ...
      query: {
        ...
        mid: '223e4567-e89b-12d3-a456-426614174000',
      },
    });
  ...
  it('should return 400 if session ID is invalid', async () => {
    const { req, res } = createMocks({
      method: 'DELETE',
      query: { id: 'invalid-uuid', mid: '223e4567-e89b-12d3-a456-426614174000' },
    });
  ...
  it('should return 400 if material ID is invalid', async () => {
    const { req, res } = createMocks({
      method: 'DELETE',
      query: { id: '123e4567-e89b-12d3-a456-426614174000', mid: 'invalid-uuid' },
    });
    ...
    expect(res._getStatusCode()).toBe(400);
    const data = JSON.parse(res._getData());
    expect(data.error).toContain('material inválido');
  });
  ...
  it('should return 403 if session is completada', async () => {
                ...
                eq: vi.fn().mockReturnValue({
                  single: vi.fn().mockResolvedValue({
                    data: {
                      ...
                      storage_path: 'sessions/test/file.pdf',
                      file_name: 'file.pdf',
                    },
                    error: null,
                  }),
  ...
  it('should return 403 if user cannot delete material', async () => {
    const { getApiUser, createServiceRoleClient } = await import('../../../lib/api-auth');
    ...
    (getUserRoles as any).mockResolvedValue([{ role_type: 'docente' }]);
        ...
        if (table === 'consultor_sessions') {
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                single: vi.fn().mockResolvedValue({
                  data: { id: 'session-123', status: 'pendiente_informe' },
                  error: null,
                }),
              }),
            }),
    ...
    expect(res._getStatusCode()).toBe(403);
    const data = JSON.parse(res._getData());
    expect(data.error).toContain('facilitador o administrador');
  });
});
