...
jest.mock('../../lib/notificationService', () => {
  const actual = jest.requireActual('../../lib/notificationService');
  return {
    ...actual,
    default: {
      ...actual.default,
      // We'll override individual methods in tests
    }
  };
});
...
describe('NotificationService Deduplication', () => {
  ...
  describe('checkForDuplicate', () => {
    test('should return true when duplicate exists', async () => {
      // Mock the Supabase response for duplicate found
      const mockFrom = jest.fn(() => ({
        select: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
        gte: jest.fn().mockReturnThis(),
        limit: jest.fn().mockResolvedValue({
          data: [{ id: 'existing-notification-id' }],
          error: null
        })
      }));

      // Override the checkForDuplicate method to use our mock
      const originalMethod = notificationService.checkForDuplicate;
      notificationService.checkForDuplicate = jest.fn(async (userId, title, description) => {
...
describe('NotificationService Event Processing', () => {
