-- GROUP ASSIGNMENTS V2 - SIMPLIFIED IMPLEMENTATION (FIXED)
...
CREATE POLICY "Consultants can view submissions" ON group_assignment_submissions
  ...
  USING (
    ...
    OR
    EXISTS (
      SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role_type = 'admin'
    )
  );
...
-- 
-- Fixed issues:
-- 1. Changed communities to growth_communities
-- 2. Changed user_profiles to user_roles with role_type field
-- 3. Added GRANT for the view