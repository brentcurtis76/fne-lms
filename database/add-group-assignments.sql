...
ADD COLUMN IF NOT EXISTS assignment_for TYPE TEXT DEFAULT 'individual' CHECK (assignment_for IN ('individual', 'group')),
...
CREATE TABLE IF NOT EXISTS group_assignment_discussions (
  ...
  assignment_id UUID NOT NULL REFERENCES lesson_assignments(id) ON DELETE CASCADE,
  group_id UUID NOT NULL,
  thread_id UUID NOT NULL REFERENCES community_message_threads(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
