...
-- 3. Create a function to sync school and client data (simplified version)
...
-- 4. Create a function to link existing schools with clients by matching name
...
-- 5. Create a simplified view for easy querying of school-client relationships
...
-- 6. Function to create a client from a school (simplified)
...
BEGIN
  ...
  INSERT INTO clientes (
    ...
    v_school.name,
    COALESCE(p_rut, 'PENDING-' || p_school_id),
    v_school.region,
    p_representante,
    p_school_id
...
-- 7. RLS policies for the new relationships
...
-- SELECT * FROM create_client_from_school(1, '12.345.678-9'); -- Creates a client for school with id=1