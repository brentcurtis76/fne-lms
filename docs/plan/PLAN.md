...
| A8 | Admin leads triage (transition-enforced via API) | **DONE 2026-08-07** (Sol PASS r1 @ `fd33706c`, zero BLOCKING) · **r3 in flight pre-merge** (owner-chosen cleanup of S-01/S-02 + 1 NIT; needs a targeted Sol re-check of that diff before merge) | `phase/a8-leads-ui` | A5, T2 |
...
| An orphaned `<label htmlFor>` on A8's terminal-state card: when a lead is `converted` the select is replaced by a `<div>`, so the label points at no element. | A8 Sol NIT | **NIT → claimed by A8 r3** |
