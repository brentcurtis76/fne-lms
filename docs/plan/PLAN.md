...
- **CI triggers on `pull_request` and `push: [main]` — and nothing else** (verified 2026-08-08). A branch pushed without a PR gets no CI at all; see the 2026-08-08 Decision Log row. Historical note, superseded by T2: gate 4 originally built and ran **only** `tests/e2e/smoke.spec.ts` against placeholder/secret env; **no Supabase stack is started or seeded in the e2e job** (the smoke spec's own comment says the synthetic tenant does not yet exist). T2 therefore *builds* the CI fixture topology; nothing may assume one exists.
