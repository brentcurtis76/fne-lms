...
| A7a | Site link rewiring + both INSPIRA flipbooks | **DONE 2026-08-07** (Sol PASS r1, zero BLOCKING; head `ebfe5f24`, code `66774a58`, 1 executor round, 1 Sol review) | `phase/a7a-links` | A6b |
...
**Scope:** `pages/index.tsx` (nav ×2, section CTA, INSPIRA flipbook button/modal/state), `pages/programas.tsx` (nav ×2, INSPIRA card link, **its own INSPIRA flipbook state/iframe**), `pages/nosotros.tsx`, `pages/noticias.tsx`, **`pages/noticias/[slug].tsx` (nav ×2)**, **`pages/equipo.tsx` (nav ×2)**, `components/Footer.tsx`. *(Scope corrected 2026-08-07 at A7a dispatch: the original list named five files, but `equipo.tsx` and `noticias/[slug].tsx` also carry `/#pasantias` nav links — **13 links across 7 files** (six pages × 2 nav entries + the one footer link; corrected from "15" 2026-08-07 at r1 verification — 15 is the count *after* the two INSPIRA CTAs become links, not the inventory that had to move). [A1] already demanded that the grep return no hits, so the criterion was unsatisfiable against the scope as written. See Decision Log.)*
...
**Criterion wording corrected 2026-08-07 at r1 verification (executor r1 finding 1 — a PM error):** the dispatch prompt restated [A1] as "`grep -rn '#pasantias' pages/ components/` returns **exactly one** hit, the preserved `id="pasantias"`". That is unproducible — `id="pasantias"` contains no `#`, so the correct-tree result is **zero**, and it was zero for the anchor on `main` before any edit too. The criterion is two separate facts and reads: **(a) `grep -rn '#pasantias' pages/ components/` returns nothing, and (b) `grep -n 'id="pasantias"' pages/index.tsx` still returns its section line.** Both are asserted independently by `__tests__/pages/pasantias-site-links.test.ts`. Anyone re-running (a) verbatim must read zero as the pass, not the failure.
...
**Added 2026-08-07 at A7a close (SOP §3.8 step 5 — plan drift caught at close): [A-new-1] the release checklist re-decides A7a's two open guard gaps rather than inheriting them silently.** A7a passed with two SHOULD-FIX findings in `__tests__/pages/pasantias-site-links.test.ts` — the Directivos flipbooks are pinned site-wide rather than per site (PM mutant M3), and no assertion pins *where* a PASANTÍAS link goes, only that it is not the old anchor (Sol S2). Both are guard reach, neither was a shipping defect at A7a's head, and both are exactly the class that goes quiet in a backlog. A9 is the last gate before Track A ships: it must either close them or record an explicit accept-as-is with a reason. The reason this is A9's and not a follow-up round's: the failure they permit — a nav link silently repointed away from `/pasantias` — is invisible to every other gate, and `/pasantias` has already spent one whole phase orphaned without the suite noticing.
...
| **The A7a link guard never pins where a PASANTÍAS link actually goes.** It rejects destinations containing `#pasantias` and pins `/pasantias` at only three of the fifteen sites (the anti-vacuity check), so repointing e.g. `pages/equipo.tsx:281` to `/programas` violates the frozen "every PASANTÍAS link → `/pasantias`" criterion with all eight guard tests and all five browser tests green. Guard reach, not a shipping defect — the current tree is correct at all fifteen. **Sol found this; the PM did not.** | A7a Sol r1 S2 | SHOULD-FIX |
| **The A7a link guard pins the Directivos flipbooks site-wide, not per page.** `matchesIn(SCOPE, id).length === 0` means the ID has to vanish from *every* file before the pin fires, so gutting the Directivos flipbook on `/programas` while it survives on `/` passes green — PM mutant M3 at r1 verification, the one survivor of five. No defect ships today (the r1 diff leaves both Directivos blocks byte-identical bar one title, verified by reading it); the pin is simply half-blind to the likeliest future accident, a partial purge. Fix is one line: assert the expected *sites* per ID, not mere presence. | A7a PM r1 mutant M3 | SHOULD-FIX |
| The A7a link guard reads only **literal** hrefs (`href="…"`, `'…'`, `{"…"}`, `` {`…`} ``). A destination built from an expression, or a `router.push('/#pasantias')`, is invisible to it. Never a false positive; a miss it cannot avoid without evaluating the source. | A7a r1 executor note + PM r1 N2 | NIT |
...
| 2026-08-07 | **A7a's scope was two files short of its own criterion (caught at dispatch, before the round).** [A1] requires `grep -rn '"/#pasantias"' pages/ components/` to return no nav/footer hits, but the scope list named five files and the real inventory is **13 links across seven files** (the headline "15" first recorded here was wrong — it counted the two new INSPIRA CTAs; the enumeration itself was complete, executor r1 finding 2): `pages/equipo.tsx` and `pages/noticias/[slug].tsx` were never listed. The criterion was unsatisfiable against the scope as written, so an executor would have had to either miss two files or edit outside its scope — the shape of miss this project has paid for three times as a propagation-class defect. Scope corrected; the criterion is unchanged. Also resolved at dispatch: [A3]'s "check for other references" (both originals are still referenced elsewhere → neither is deleted), and the grep proofs are promoted to a derived source guard plus e2e on an already-mandatory spec. | A7a dispatch inventory | Fable (PM) |
