# PLAN — INSPIRA Comms (Pasantías Barcelona web/leads + Email platform) — v3
...
- PLAN REVIEWS: v1 FAIL (13 BLOCKING — REVIEW-PLAN.md); v2 FAIL (8 BLOCKING — REVIEW-PLAN-R2.md; 8/13 r1 findings CLOSED, arbitration taken to Brent). This v3 implements the four arbitrated decisions + the four mechanical R2 defects. Triage: LEDGER round 3.
- PLAN FROZEN: — (DRAFT; freezes on Codex confirmation PASS)
...
| B1a | Expense-report mail moved server-side (recipients derived server-side) | TODO | `phase/b1a-expmail` | — |
| B1b | Relay removal (`send-email.ts`, `test-email.ts`, sendgrid dep) | TODO | `phase/b1b-relay` | B1a |
...
As v2 A3, with evidence location fixed: rendered PNGs of every page at 144 DPI committed to `docs/plan/evidence/a3/` and linked from the report/ledger; PM inspects from the repo, not chat. **Other criteria unchanged** (text-extraction tests: brochure has "1.560", ficha has no monetary tokens; RFC 5987 filename constants).
...
**Acceptance criteria:**
- [A1] Tables per D-06 including: `email_contacts.anonymized_at` + the two-shape identity CHECK (normal vs anonymized-NULL); `email_campaigns.status` CHECK **(`draft|sending|sent|sent_with_errors`)**; `email_campaign_sends` FK `ON DELETE RESTRICT` + UNIQUE(campaign,contact) + indexes; `email_suppression(email_hash PK, reason, created_at)`; `email_webhook_events(svix_id PK, event_type, resend_email_id, payload jsonb, received_at)`.
...
- [A2] `anonymize_email_contact(uuid)`: per D-06 contract (NULL identity set + `anonymized_at` + sends' email snapshots NULLed + tombstone insert, reason `manual`); **idempotent second call is a tested no-op**; metrics unchanged by anonymization (asserted on a seeded fixture).
- [A3] `process_webhook_event(p_svix_id, p_type, p_resend_email_id, p_payload)`: **single-transaction dedup + effect** per D-08's event table (incl. `sent`/`delivery_delayed` ledger-only no-ops); returns `inserted|duplicate`; **injected failure mid-effect rolls back the svix row too** (asserted by wrapping in a failing transaction and re-processing successfully).
...
- [Δ1] `detectProtectedAmounts(text)` helper (protected INSPIRA literals from the **server-held** list — the client receives only a boolean/regex-safe derivative, not the commercial module) powering a **non-blocking** es-CL warning banner in the preview component; warning render tested; nothing blocks.
...
- [A2] E2E (mandatory, admin fixture): create → edit → preview renders in sandbox → dirty-guard prompt → price warning appears when a protected amount is typed (non-blocking). Gates green.
