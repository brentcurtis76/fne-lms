...
| A8 | Admin leads triage (transition-enforced via API) | **DONE 2026-08-07** (Sol PASS r1, zero BLOCKING; head `fd33706c`, 2 executor rounds, 1 Sol review) | `phase/a8-leads-ui` | A5, T2 |
...
| An orphaned `<label htmlFor>` on A8's terminal-state card: when a lead is `converted` the select is replaced by a `<div>`, so the label points at no element. | A8 Sol NIT | NIT |
