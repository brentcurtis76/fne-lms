...
| 2026-08-13 | **Q6 RULED — both candidates PROMOTED to `CLAUDE.md`.** (1) the mirror precedence clause "CLAUDE.md wins — fix the divergence in the same PR"; (2) "+ per-role testing" for middleware/RBAC changes. Neither is retired. | Candidate 1's reject branch was self-defeating — retiring it would delete the very invariant RM exists to restore — and `CLAUDE.md:4` already carried a partial form ("`AGENTS.md` mirrors this file"), so only the precedence-and-same-PR clause was actually missing. Candidate 2 is consistent with the repo's existing per-role convention (RLS policies tested per role × table × operation) on the area both files call the most bug-prone; retiring it would have weakened a testing requirement. `evidence/RM-retired-guidance.md` is therefore **not created** — nothing was retired. | Brent |
| 2026-08-13 | **RM's `STATUS: BLOCKED` was a misdiagnosis, corrected by PM verification.** The build failure was a missing gitignored `.env.local` in the worktree, not a base failure; `npm run build` passes once it is present. | The executor reproduced at merge base **inside the same env-less worktree**, which can only show both failing identically — it cannot separate a code regression from a missing environment. The correct control is a checkout that *has* the environment. **Recorded as a method finding of the same shape this plan has hit repeatedly: a control that cannot distinguish what it exists to distinguish.** No overlay §5 stabilization phase is needed. | PM verification |
| 2026-08-13 | **D-8 WAS VIOLATED IN RM's EXECUTION.** `PROJECT_STATE.md:33` records the RM executor as **Codex**, but D-8 freezes Codex as reviewer and reserves implementation for a fresh Claude Code executor. | Detected by the PM at the point of requesting RM's independent review. **The substantive work is not in doubt** — the PM verified the mirror invariant, both Q6 promotions, all nine criteria and the artifacts independently. The defect is procedural, and its live consequence is narrow but real: **the reviewer of RM must not be the agent that executed it**, or the independence the loop exists for is gone. Re-executing correct, verified work would be theater; the repair is at the review step. **Whoever reviews RM must have no execution context for it.** | PM |
