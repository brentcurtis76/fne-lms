...
**Current status (documentation update recorded 2026-09-05 UTC — evening of 2026-09-04 in Chile — see §9): RELEASED. Merged to `main` via PR #83 (merge commit `8f5bbe6d7be833a31d033032dc9ddf75988be775`, 2026-09-04T21:28:07Z); post-merge CI run completed successfully (seven jobs); Production deployment metadata reports success (GitHub deployment `6273190857`, 2026-09-04T21:31:34Z). Live Production functionality NOT verified. B2c, W-B2c-01 and the eight-table RLS remediation NOT complete.** *Historical status pointer at the §8 closure step (2026-09-04, retained verbatim; its "NOT pushed / merged / deployed" statements were true when written and are superseded by §9):* **INDEPENDENT REVIEW APPROVE_WITH_NOTES (Codex, 20:30 UTC) — no blocking findings remain; AUTHORIZED FOR ONE LOCAL COMMIT by Brent. Still: B2c NOT complete; CI NOT verified; NOT pushed, NOT merged, NOT deployed, NOT verified in Production.** *Historical status at the R1 hand-off (retained verbatim):* LOCAL REMEDIATION REPORTED (R1) — awaiting independent Codex rereview. Not self-approved; review NOT approved; B2c NOT complete; CI NOT verified; NOT committed, pushed, merged, deployed or verified in Production. The original hand-off (2026-09-04, "LOCAL IMPLEMENTATION REPORTED") received an independent Codex verdict of **REQUEST_CHANGES** with one blocking P2 finding: the two new test files were created under `tests/api/cron/` although the authorized paths were `__tests__/api/cron/` (the directory that holds the existing `auth-retention.test.ts` example named in the brief), and this document wrongly listed the substituted paths as the authorized allowlist. Remediation round R1 (this revision) relocated the two files and corrected this document and `PROJECT_STATE.md`; nothing else changed. See §2 and §5.
...
- At the R1 hand-off no commit existed; the PM/reviewer saw this work only as the uncommitted diff in the named worktree. `git diff --check` is clean. (Superseded by §8: one local commit was subsequently authorized; and by §9: that commit was published, merged and deployed.)
...
**Not verified by anyone for this change:** database (`test:db`), Production, and CI. No database, Production or CI verification was performed. *(True at the §8 checkpoint. §9 records the two later CI runs — the PR CI run on the approved source commit and the post-merge CI run on the merge commit — which supplied the database/E2E evidence for this change; that evidence is CI-only, and live Production functionality remains unverified.)*
...
## 9. Release closure — publication, merge, CI and Production deployment metadata (events of 2026-09-04 UTC; recorded 2026-09-05 UTC)

**Nature of this section.** Documentation-only record written by a fresh, bounded Claude Code session on 2026-09-05 (UTC; evening of 2026-09-04 in Chile), after Codex's independent release review and on Brent's authorization. Every event timestamp below is the GitHub-recorded time of the event itself, not the time of this record. No exact timestamp is recorded for Codex's own review steps because none was captured; they are dated 2026-09-04 only.

**Verdict layers.**

| Layer | Outcome |
|---|---|
| Implementation (§8) | `APPROVE_WITH_NOTES` — Codex, independent |
| Publication (push of the single authorized local commit; PR #83) | independently approved by Codex |
| Release (merge to `main`) | independently verified by Codex — `APPROVE_WITH_NOTES` |
| Merge and Production deployment | **evidenced by metadata** (below) |
| Live Production functionality | **NOT verified** — the running application was not opened or exercised by anyone for this release |

**Source commit.** The one local commit authorized in §8 is `8e7e76781faeced899ce87cfb1120a5adaf1cd4a` (`fix(auth): secure learning-path maintenance endpoints`), whose parent is the approved base `8b58121779eb744c790538b517db7cf023ad1da1` and whose content is exactly the six files of §2. The worktree `/Users/brentcurtis/dev/wt/rls-learn` on `fix/rls-learn` was clean during release verification and again immediately before this documentation update; its local HEAD remains that commit, intentionally **not** synchronized with `main`, and the remote `fix/rls-learn` is the same SHA. This documentation update leaves exactly two unstaged, uncommitted modified files in that worktree (this document and `PROJECT_STATE.md`); no other change was made.

**Merge — independently verified by Codex (2026-09-04).** This documentation session re-read the same GitHub metadata read-only on 2026-09-05 UTC (PR, commit, workflow run and deployment endpoints; no fetch, no checkout, no application access) and obtained identical values:

| Fact | Value |
|---|---|
| Pull request | `https://github.com/brentcurtis76/fne-lms/pull/83` — state MERGED, not draft; base `main`, head `fix/rls-learn` |
| Approved source commit (PR head, second parent) | `8e7e76781faeced899ce87cfb1120a5adaf1cd4a` |
| Merge commit | `8f5bbe6d7be833a31d033032dc9ddf75988be775` |
| Merged at | `2026-09-04T21:28:07Z` |
| First parent | `8b58121779eb744c790538b517db7cf023ad1da1` (the approved base) |
| Merge tree | `c17850f46bb33c7542dd275412c460c1f2f97378` — identical to the approved source commit's tree (also confirmed locally: `git rev-parse HEAD^{tree}` in the worktree gives the same SHA) |
| Live `main` at verification | pointed at the merge commit (Codex, 2026-09-04; re-confirmed via `git ls-remote` on 2026-09-05 UTC) |
| Local source branch | HEAD unchanged at the approved source commit; clean at release verification and before this documentation update (historical checkpoints); after this update, exactly two unstaged, uncommitted modified files (this document and `PROJECT_STATE.md`) |

**PR CI (event `pull_request`, head SHA = the approved source commit `8e7e76781faeced899ce87cfb1120a5adaf1cd4a`):** run `https://github.com/brentcurtis76/fne-lms/actions/runs/33918082655`, independently verified; Gate 3 — RLS pgTAP (`supabase test db`) and Gate 4 — E2E (Playwright on seeded local Supabase) passed.

**Post-merge CI (event `push`, head SHA = the merge commit):** run `https://github.com/brentcurtis76/fne-lms/actions/runs/33921234076`, independently verified; status completed, conclusion **success**. Seven jobs, all successful: Migration safety guard; Browser/server boundary guard; Gate 1 — Typecheck; Gate 1b — Lint; Gate 2 — Unit (Vitest); Gate 3 — RLS pgTAP (`supabase test db`); Gate 4 — E2E (Playwright on seeded local Supabase).

**Production deployment (metadata only):** GitHub deployment `6273190857`, environment `Production`, SHA = the merge commit, status `success` at `2026-09-04T21:31:34Z`. The deployment metadata reports success; **the application was not opened or exercised**, by Codex or by this session.

**What was released, and what is not claimed.** The released change authenticates the two learning-path maintenance endpoints before any database operation: missing/empty/whitespace-only `CRON_SECRET` → 503; missing or incorrect exact `Bearer` authorization → 401; existing methods and the authenticated processing path unchanged (§3–§4). It changed **no** database policies, grants, functions or migrations. Still unverified or unresolved, and **not claimed** by this record: the Production `CRON_SECRET` configuration; the missing RPCs and `learning_path_*_summary` relations (§7); the fact that neither route is scheduled; any live endpoint behaviour, any authenticated maintenance success, and any Production database correctness. **Overall B2c, W-B2c-01 and the eight-table RLS remediation remain incomplete.** The governed `santa-marta-work-items.csv` row for W-B2c-01 was deliberately not touched by this update; any broader ledger change is a separate decision outside this documentation authorization.

**Validation evidence — distinctions retained exactly as in §5 and §8.** Codex focused command 81/81, with type-check, lint and build passed. Claude R1 full suite: 8,882 passed / 12 skipped, exit 0 (executor-reported). Codex full suite: 8,881 passed, one reviewer-induced `NEXT_PUBLIC_BASE_URL` failure, 12 skipped; the affected suite then passed 13/13 without the override. Those two full-suite records are **not** a single clean independent full-suite run. Database (pgTAP) and seeded E2E evidence for this change comes from two independently verified CI layers: the PR CI run `33918082655` (event `pull_request`, approved source commit `8e7e76781faeced899ce87cfb1120a5adaf1cd4a`), where pgTAP and seeded Playwright passed, and the post-merge CI run `33921234076` (event `push`, merge commit `8f5bbe6d7be833a31d033032dc9ddf75988be775`), where they passed again. These are CI-only checks; no local database/E2E testing and no live Production testing was performed for this review by anyone.

**Procedural notes — both non-blocking, both on record, neither a defect in the released code.**

1. *Directory removal (§8, retained):* the R1 executor removed the two empty `tests/api/cron/` and `tests/api/` directories despite an instruction not to clean directories; reviewed files intact; deviation recorded, not retroactively authorized, no recreation requested.
2. *Release step:* the release executor reused an existing Claude conversation despite the explicit fresh-session instruction and reported "no deviations". Codex identified that statement as inaccurate. Independent verification found the released artifact correct (merge tree identical to the approved source tree); no release correction or rollback was required. This is **not** retroactive authorization of the deviation.

**Authorization and limits of this record.** Brent authorized this documentation-only update — this section, the current-status pointers of this document, and the B2c-M1 entry of `PROJECT_STATE.md` — after the independent release review. The session performed no fetch, commit, push, PR change, deployment, branch/worktree change, database access or live-endpoint access; its only external activity was the read-only GitHub metadata reads described above. At the time of writing these edits are **uncommitted working-tree changes** in the worktree; no commit SHA and no publication of them is claimed.

— Recorded 2026-09-04 by Claude Code (sole implementation writer for B2c-M1); revised 2026-09-04 in remediation round R1 by a fresh Claude Code session after Codex's REQUEST_CHANGES verdict; §8 added 2026-09-04 by a fresh Claude Code session (bounded closure step) recording Codex's APPROVE_WITH_NOTES verdict and Brent's one-local-commit authorization; §9 added 2026-09-05 (UTC) by a fresh Claude Code session (bounded documentation-only closure) recording the independently verified merge, CI and Production deployment metadata. Independent reviewer: Codex. *Historical closing status at the §8 step:* APPROVED WITH NOTES — AUTHORIZED FOR ONE LOCAL COMMIT; not pushed, merged, deployed or Production-verified; B2c not complete. **Current closing status (§9): RELEASED — merged to `main` via PR #83, post-merge CI successful, Production deployment metadata successful; live Production functionality not verified; B2c, W-B2c-01 and the eight-table RLS remediation not complete.**
