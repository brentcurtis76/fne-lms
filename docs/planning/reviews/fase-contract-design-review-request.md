...
- Safari verification: rendered saved PDFs at full size. Short anexo is one page and the 30-payment anexo is three pages, both with intact signatures/footer. The short anexo initially looked incomplete to `pypdf` text extraction, but raster inspection confirmed all content; this was an extraction limitation, not missing visible content.
- Fixed a real Safari contrato pagination issue: an avoided closing block crossing the final page was omitted. A WebKit feature query now starts the final three contract clauses on a new page. The Safari proof is six Letter pages with the final clauses, both signatures, and footer together. Chromium remains five pages. This deliberate browser-specific page-count difference preserves the approved typography and spacing.
- Native Safari proof files: `safari-annex.pdf`, `safari-contract-verified.pdf`, `safari-long-annex-verified.pdf` in the external evidence directory. Unsuccessful diagnostics are retained separately and are not release proofs.
- Draft PR: https://github.com/brentcurtis76/fne-lms/pull/91. Await final-head CI before marking ready. No production database operations or main merge.
