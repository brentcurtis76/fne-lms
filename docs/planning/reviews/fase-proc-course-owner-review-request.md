...
**Original implementation base:** the Base SHA above, `804794df…` — preserved as the evidence base of the implementation, correction and assurance passes · **Integration base (local rebase, 2026-09-02, Brent-authorized integration-preparation pass):** `8218e597e148d8044fe7d330c118243aa3772485`, live `main` at rebase time · **Approved code head:** `5f0e9b1b4e39e88565a28f873bca5f1c6bf01a76`, rebased as `7ac8d8b477d0e7973785b976e9963681cf11bbbc`; production code byte-identical in both · **Current branch head:** recorded in the completion report, because this document cannot embed the SHA of its own latest modifying commit · **Commits:** 5 over the integration base — the four rebased C-01 commits (old→new mapping in *Local integration preparation*) plus the integration-preparation record, which is this document's latest modifying commit. Historical chain over the original base: implementation + tests `5f0e9b1b`; this review request `0557a7c1`; the evidence/documentation correction `a747de74`; the final assurance pass `9205fd4a` (adversarial tests and this document only; no production code)
...
**Why this section exists.** The first version of this document classified the 60 `npm run e2e` failures as "the known pre-existing state" of the legacy suites on the strength of memory and failure signatures, and said so was *not proven by a base run*. Brent directed an evidence-only correction: reproduce the run on the exact base and compare. This section records that comparison; no production code, test, migration, CI, or package file changed, and `fix/course-owner` was not rebased in that pass (the later, separately authorized local rebase is recorded under *Local integration preparation*).
...
Recorded scope, not broadened: the exception applies **only** to the 60 literal `CI=1 npm run e2e` failures reproduced identically on the exact base and on the branch (see *Base-versus-branch full E2E comparison*). The full command **remains red** (exit 1) on both; nothing in this pass changes that. The **mandatory CI Playwright gate remains required** and was re-run green in this pass. **No integration or external action was authorized** by that exception: no rebase or merge from main, no push or remote branch, no PR, no merge, no deployment, no production, database, provider, secret or PII access, no production cleanup, no worktree removal, no C-02 replacement behaviour, no D-01 constraint, no consultor or RLS change. (The local rebase recorded under *Local integration preparation* rests on a **later, separate** authorization from Brent, not on this exception; the exception's scope is unchanged.)
...
**Update (integration-preparation pass, 2026-09-02).** The paragraph above remains accurate for the implementation, correction and assurance passes: the branch was **not** rebased during any of them. Under Brent's later, separate and explicit authorization ("Authorize the local C-01 integration-preparation pass") it **was rebased locally** onto `8218e597` — see *Local integration preparation* below for the command, the old→new SHA mapping, the patch-equivalence proof and the re-run gates. Nothing was pushed, merged or deployed by that pass either.

## Local integration preparation (2026-09-02, Brent-authorized)

**Authorization.** Brent: "Authorize the local C-01 integration-preparation pass" — following the PM recommendation to update the local C-01 branch onto the latest main and rerun the important checks. Authorized: the local rebase, the local gates, this documentation update and one documentation-only commit. **Not authorized and not performed:** push or remote branch, PR creation/edit/comment, merge, deployment or production verification, production database / Supabase Management API / Vercel / provider / secret / PII access, cleanup or provisioning, any change to production code, tests, migrations, CI, package files, Playwright configuration or E2E seeds, C-02 / D-01 / D-02 / H-02 / replacement / DELETE / RLS / consultor changes, worktree cleanup, touching the primary checkout or its `.env.local`, or resolving conflicts or failures by modifying code.

**Preflight (read-only), all as expected:** worktree clean on `fix/course-owner` at `9205fd4ab36be9df0fd1dfa2142ef56634add2bb` (parent `a747de74`); exactly four commits over `804794df`; the five-file scope; live `refs/heads/main` = `8218e597e148d8044fe7d330c118243aa3772485` and no remote `fix/course-owner` (`git ls-remote`); the original-base→live-main delta = exactly `PROJECT_STATE.md`, `docs/planning/reviews/fase-cred-m1-close-review-request.md`, `docs/runbooks/auth-security.md` (3 commits; confirmed both by the read-only GitHub compare API and by `git diff --name-only 804794df 8218e597` — the `8218e597` object was already present in the shared repository as `origin/main`, so this pass ran **no fetch**); open PRs #50, #46, #4, #1 with no C-01 file among their changed files (`gh pr view --json files`); no active git hooks.

**Rebase.** `git rebase --onto 8218e597e148d8044fe7d330c118243aa3772485 804794df02b4165f58d89fe77649e1d71423d7dc fix/course-owner` — non-interactive, no conflict, no squash, amend, reorder or edit. Parent of the first rebased commit: `8218e597`. Exactly four commits above it.

| Historical commit (over `804794df`) | Rebased commit (over `8218e597`) | Subject |
|---|---|---|
| `5f0e9b1b4e39e88565a28f873bca5f1c6bf01a76` | `7ac8d8b477d0e7973785b976e9963681cf11bbbc` | fix(assessments): contain multiple active docentes per course and require exact-school eligibility (C-01) |
| `0557a7c1dd30391c8b3b60e45f5b2393bde5b872` | `fb918bd0fbd33a9590caf1611ff9558c64a2e5e5` | docs(reviews): add PROC-COURSE-OWNER-01 review request |
| `a747de741c66dd1b2ab252d7878a338b7ebe741d` | `0290239d845e864a998881b1cccd20b1ae733a07` | docs(reviews): correct C-01 evidence — base-vs-branch full E2E comparison, red-gate status, read-only GitHub access |
| `9205fd4ab36be9df0fd1dfa2142ef56634add2bb` | `aaec287f6715c572969394bf0fc3cbff555dae37` | test(assessments): adversarial C-01 edge tests and final assurance record |

**Equivalence and scope.** `git range-diff 804794df..9205fd4a 8218e597..aaec287f` classifies all four commits as patch-equivalent (`=`). `git diff --name-only 8218e597 aaec287f` is exactly the five C-01 paths: `__tests__/api/school/assign-docente.test.ts`, `__tests__/pages/school/transversal-context-assign.test.tsx`, `docs/planning/reviews/fase-proc-course-owner-review-request.md`, `pages/api/school/transversal-context/assign-docente.ts`, `pages/school/transversal-context/index.tsx`. Production code is byte-identical to the approved tree at `5f0e9b1b` (empty diff for the two production files, and for `pages lib utils components types supabase scripts package.json package-lock.json .github playwright.config.ts tests`). Migrations, CI, package files, Playwright configuration, seeds and unrelated tests are unchanged from the approved state `9205fd4a` (empty diff over those paths).

**Gates re-run on the rebased tree** (`aaec287f`, in `~/dev/wt/course-owner`):

| Gate | Command | Result |
|---|---|---|
| Focused Vitest | `npx vitest run __tests__/api/school/assign-docente.test.ts __tests__/pages/school/transversal-context-assign.test.tsx` | **81 passed** (60 API + 21 page), exit 0 |
| Whitespace | `git diff --check` | exit 0, clean |
| Actions guard | `npm run guard:actions` | exit 0 — 17 uses across 1 workflow file OK |
| Migrations guard | `npm run guard:migrations` | exit 0 — no RLS disable; 40 migration files, no DROP/TRUNCATE/destructive ALTER |
| Browser guard | `npm run guard:browser` | exit 0 — 1142 files, 686 modules from 509 page entrypoints, no boundary violation |
| Secrets guard | `npm run guard:secrets` | exit 0 on the rebased tree — 2465 tracked paths (main's new cred-close review request included), 0 findings; re-run with this document staged and again after the commit: reported in the completion report |
| Typecheck | `npm run type-check` | exit 0 |
| Lint | `npm run lint` | exit 0 (`--max-warnings=0`) |
| Testid lint (advisory) | `npm run lint:testid` | **exit 1 — 2623 problems (44 errors, 2579 warnings), exactly the documented repository baseline; red, not green**; unchanged because neither C-01 nor the three main commits touch `pages/` or `components/` |
| Unit | `npm test` | exit 0 — **374 files passed (374); 8621 tests passed, 11 skipped (8632)**, 262.7 s — the expected counts, unchanged by the rebase |
| Build | `npm run build` | exit 0 — ✓ Compiled successfully, ✓ Generating static pages (149/149), local-stack env inlined |
| Price leak | `node scripts/check-price-leak.mjs` | exit 0 — OK, 262 files under `.next/static`, no commercial data |
| pgTAP | `supabase db reset` → `npm run test:db` | reset of the local Docker stack from migrations exit 0 (42 s, 40 migrations); `supabase test db`: **Files=24, Tests=1931, Result: PASS**, exit 0 |
| Mandatory E2E | `node scripts/ci/seed-e2e.mjs` → `CI=1 npx playwright test $(node scripts/ci/e2e-mandatory.mjs --list) --project=chromium` → `node scripts/ci/e2e-mandatory.mjs --check test-results/e2e-results.json` | seed exit 0 (synthetic fixtures, local stack only); 13-spec manifest under Chromium against the production build of the rebased tree: **192 passed (2.5 min), 0 failed, 0 flaky, 0 skipped** (`test-results/e2e-results.json`: expected=192 unexpected=0 flaky=0 skipped=0), exit 0; checker: **OK — 13 mandatory spec(s) ran with no skips**, exit 0. Green and required; regression evidence only |
| Literal `CI=1 npm run e2e` | — | **not re-run**; still **red** (exit 1 on the exact base and on the C-01 code head, 60 identical failures). Conditions for not re-running were verified: production code and every relevant E2E input (`supabase/migrations/`, `tests/`, `playwright.config.ts`, `scripts/ci/`, `package.json`, `package-lock.json`, `pages/`, `lib/`) are byte-identical to the approved C-01 state. Brent's existing exception remains limited to those 60 reproduced failures and is **not broadened** by the rebase. |

**Local-stack coordination (before any reset or Playwright run, read-only):** only the existing local Docker stack (`supabase/config.toml` project id, `127.0.0.1`) was used; the worktree's existing local-only `.env.local` was consumed without printing, copying or inspecting any key value. Checks: no listener on TCP 3000; no `next`, Playwright, Supabase CLI, Vitest or seed process from any other checkout; zero non-idle client sessions on the local Postgres (a read-only `pg_stat_activity` query). A first attempt at 18:28 UTC found another session's `vitest run` in the unrelated worktree `~/dev/wt/sm-sim-d0` and **stopped before any reset** (`LOCAL_STACK_COORDINATION_REQUIRED`, nothing killed); the executor waited read-only for that process to exit on its own (18:29:31 UTC), re-ran the same check — clean — and only then reset the stack. Another session was concurrently active in the primary checkout and other worktrees; none of its processes used the stack or the port at reset or Playwright time.

**Second live-main check before this record:** read-only `git ls-remote --heads origin` at 18:33:13 UTC, after every gate above — `refs/heads/main` still `8218e597e148d8044fe7d330c118243aa3772485`, no `refs/heads/fix/course-owner`. Should main move after this record, the branch is not to be chased or rebased again without a new authorization.

**Preserved, unchanged:** Brent's exact E2E exception (quoted verbatim under *Human gate decision*) and all four known limitations — the concurrent different-docente POST race until D-01; manual DELETE then POST unsafe until C-02; no C-01 Playwright journey yet (mandatory E2E is regression evidence only); consultor semantics (H-02) deferred.

**Nothing external.** No push, no remote branch, no PR, no merge, no deployment, no production access, no provider mutation, no secret access, no cleanup was authorized or performed. Both C-01 worktrees (`~/dev/wt/course-owner` and the detached evidence worktree `~/dev/wt/course-owner-base` at `804794df`) are left in place. The integration-preparation record is this document's latest modifying commit; its SHA is reported in the completion report, not embedded here.

## Access confirmation

**Read-only GitHub access occurred** — reads of remote state, never writes: `git ls-remote --heads origin` (preflight, after each commit, again in the correction pass, at the start and end of the final assurance pass, and at the start and end of the integration-preparation pass), one `git fetch --dry-run origin main` (implementation preflight; a dry run updates nothing), `gh pr list --state open` (the overlap check at preflight, in the final assurance pass, and in the integration-preparation pass), `gh pr view <n> --json files` for the four open PRs (final assurance and integration-preparation overlap checks), and `gh api repos/brentcurtis76/fne-lms/compare/804794df…8218e597` (twice after main moved, and once more in the integration-preparation preflight). The integration-preparation pass ran no fetch: the shared repository already held `8218e597` as `origin/main`. The rebase rewrote only the local ref `refs/heads/fix/course-owner`. **No GitHub mutation occurred**: no push, no remote branch, no PR, no comment, no merge, no label or settings change; `fix/course-owner` exists only locally. No production database, Supabase Management API, Vercel, provider, or secret-state access occurred. The primary checkout's `.env.local` was never read or copied. The only environments touched were the local Docker Supabase stack and the isolated worktrees under `~/dev/wt/` (the branch worktree and, for the correction pass, a detached base worktree).
