...
| 26 | ~~Zoom-side recording residue~~ — **CLEANED**: G2 settled, so both retained stop-control recordings (466 917 B and 471 953 B, 6 files each) were permanently deleted; `GET recordings` returns *"Esta grabación no existe"* for both. The 6 empty `PRUEBA SPIKE — no unirse` scheduled meetings remain and are harmless | Closed 2026-07-30 |
| 27 | **The webhook subscription in Marketplace now points at a DEAD tunnel URL.** The `cloudflared` quick tunnel and receiver were stopped after capture, so the endpoint is unreachable. Zoom re-validates every 72 h and disables a subscription after 6 failures (§18), which is the expected and harmless outcome for a spike — but if Brent wants the subscription kept alive he must repoint it, and **Z1b will need to repoint it at the real route regardless** | Brent / Z1b |
