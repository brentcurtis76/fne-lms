...
| A14-4-F04 (F4) | P2 | LOCALLY_APPROVED | SM-07 distinguishes absent hours records from recorded statuses across screen/CSV/PDF; shipping separate |
| A14-4-F05 (F5) | P2 | LOCALLY_APPROVED | SM-08 marks capped session detail explicitly partial across UI/CSV/PDF; totals preserved; shipping separate |
...
SM-07 local review 2026-09-18: Brent approved explicit missing-hours-record representation. After successful ledger read, absent entries are report-only sin_registro; screen says Sin registro de horas with Horas programadas, CSV/PDF say Sin registro de horas (horas programadas). Recorded statuses/zero waivers/fractional hours and all totals remain authoritative and unchanged; no database enum or write. PM240focusedpass; matching-lock executor435files/10143fullpass12existing skips, type/lint/build pass. r0 named20browser checks and PM23counterexamples pass (legacy whole-school PDF plus enlarged tablet/mobile scrolled zero comparison); final manifest browser evidence is linked in /home/brent/Projects/pm-workflow/runs/SM-07/pm-review-r1.md. One correction round updated a stale no-ledger status expectation and report counts; no tests removed or weakened. F04 locally complete; F03 broader completeness/F05-F12 and parent remain held, shipping/institutional acceptance separate.

SM-07 routed advisories: existing KPI labels overlap at200% root text size (PM enlarged-tablet screenshot), while the new status/caption remain fully reachable by inner table scrolling; owner next authorized F10 presentation/accessibility unit, trigger its scoping. Review-request fails-before prose calls both old expectations consumida although the first was reservada; authoritative r0 diff/tests preserve the distinction. Correct this harmless historical wording with the next parent documentation update. Executor repeated full gates after documentation-only edit despite once-final policy; no evidence invalidated, reinforce reuse for documentation-only changes in next dispatch.

SM-08 local review 2026-09-18: explicit partial-detail reporting selected by Brent and verified across screen/CSV/PDF. Service collects at most501 rows to emit at most500 latest sessions per category; stable date/id order and short-page progress distinguish499/500/501 correctly. Oversized, malformed, repeated and failed pages fail closed; sentinel never enters ledger lookup or emitted detail. UI warning remains visible while collapsed; CSV adds one non-numeric notice row per truncated category, retaining17columns and summary first; PDF repeats the warning with the affected table across pages. Totals and hours semantics unchanged. PM269focusedpass and independently rerun full435files/10172pass12existing skips, type-check/lint/build/diff-check pass. Named19 and PM20browser assertions pass, including actual multipage PDF/CSV and all-tied dates with one-row server pages. Final manifest receipts and exact locks: /home/brent/Projects/pm-workflow/runs/SM-08/pm-review-r1.md. One correction round rejected oversized responses before row iteration; no tests weakened. F05 locally complete; parent/F03 broader completeness/F06–F12 remain held, shipping and institutional acceptance separate.

SM-08 evidence advisories: executor r1 omitted exact final fingerprints, so PM independently reran every required gate against a frozen lock; future dispatch/report checks must retain exact after-lock values. Review-request diff-stat description includes the PM ledger in tracked totals and excludes the untracked review document; authoritative approved manifest/PM review governs scope. Route that harmless wording to the next parent documentation update, with the existing SM-07 prose advisory.

Next-item repo conflict: F06's historical source description predates the existing request sequence guard now in components/hours/SchoolHoursReport.tsx:434–461. Do not reimplement from stale prose. A separately authorized verification of delayed success/error, rapid school switching and actual CSV/PDF identity is needed before F06 closure; SM-08 only verified its own warning cleanup scope.
