...
## 14. Corrección de semántica global de rutas de aprendizaje (2026-08-29)

**Decisión del dueño (Brent, 2026-08-29).** Esta sección registra la decisión de semántica que §13.8 dejaba explícitamente abierta, y su efecto sobre los ledgers y el validador. Supersede en tiempo presente la consecuencia de gobernanza de §13.6–§13.7 — la cadena «clasificación B ⇒ reparación clase 3 W-B2d-01 antes de B2c» — **sin tocar una sola cifra ni una sola palabra de la evidencia**: la cronología, las cinco consultas, la lectura de metadatos y los agregados de §13 permanecen íntegros y correctos. El registro congelado no se toca: **160 reclamaciones, 36 P0, texto de reclamación byte-idéntico, SHA-256 del legacy intacto** — y desde esta corrección el validador ancla además el SHA-256 de la propia instantánea congelada de reclamaciones.

### 14.1 Las decisiones autoritativas

1. Las filas de `learning_paths` son **plantillas globales FNE**. 2. **Ningún colegio es dueño de una ruta de aprendizaje.** 3. Las rutas **no son específicas de generación**. 4. `school_id` y `generation_id` NULOS en las filas existentes son **alcance global intencional**, no datos de propiedad ausentes o corruptos. 5. Una ruta puede asignarse por la matriz de asignación a usuarios o grupos **de cualquier colegio**. 6. La asignación afecta **disponibilidad y uso**; no convierte la ruta en propiedad de un colegio ni de una generación. 7. Solo el **rol RBAC literal `admin`** crea, edita, borra, asigna o desasigna rutas. 8. **Ningún otro rol** tiene esas acciones de gestión — expresamente incluidos `equipo_directivo` y `consultor`. 9. Quien recibe una ruta por asignación válida (directa o por grupo) la **consume** y actualiza **solo su propio progreso permitido**. 10. Estas decisiones **no autorizan** implementación ni acceso a producción.

### 14.2 Por qué la inferencia anterior era incorrecta — y qué se conserva

La clasificación **B** de §13.6 descansaba en una premisa: que toda ruta debía tener un colegio dueño derivable (un tenant por el que aislar), de modo que el alcance 100 % NULO, los roles de creador no autoritativos y la dispersión multi-colegio de las asignaciones probaban filas *rotas*. La decisión del dueño rechaza la premisa: los NULOS **representan el alcance global previsto**, y con la semántica correcta los mismos agregados describen **datos sanos** — 7 plantillas globales, 22 vínculos de curso que heredan alcance global, y 883 asignaciones directas que *deben* poder abarcar colegios porque la asignación gobierna disponibilidad, no propiedad. **Conclusión efectiva: clasificación A — ninguna transformación de datos de rutas de aprendizaje existentes es necesaria.** La clasificación B **no se borra ni se reescribe**: queda como registro histórico correcto de la inferencia hecha el 2026-08-28 (§13.6 y §6 del registro de evidencia), y la supersesión vive como sección fechada §8 del registro de evidencia — no se finge que la clasificación B nunca se alcanzó.

### 14.3 Efecto sobre los ledgers

- **`W-B2d-01`** pasa a **`SUPERSEDED`** — retirado **sin ejecutar**: el backfill propuesto no se diseñó, no se autorizó, no se ejecutó y no se representa como completado; `authorization_status` permanece `UNAUTHORIZED`; **deja de ser prerrequisito de B2c**. No es `DONE` (mentiría: nada se ejecutó) ni sigue `BLOCKED` (nada espera desbloquearlo). Conserva su forma histórica — lote `B2d`, rama `data/lp-scope`, `MERGE`, clase 3, mapeo único a `SWEEP-MI-APRENDIZAJE-09` — como registro de lo que se propuso; su gate queda marcado como registro histórico que no debe ejecutarse. **Juicio de vocabulario de estados:** el enum gobernado (`SCHEDULED`/`ACTIVE`/`BACKLOG`/`BLOCKED`/`DONE`) no podía representar con verdad un ítem retirado sin ejecutar; la extensión mínima coherente es **un** valor terminal aditivo, `SUPERSEDED` («retirado sin ejecutar por rechazo de premisa del dueño»), añadido al validador y a la contabilidad del protocolo. Ningún valor existente cambia de nombre ni de significado.
- **`W-B2c-01`** se **reclasifica a clase 2** (corrección de permisos/RLS y de fronteras de función y API) y permanece **`BLOCKED`, sin autorizar para implementación**, con exactamente tres prerrequisitos, en orden: (1) la corrección de gobernanza del 2026-08-29 aprobada de forma independiente y mergeada; (2) Privacidad aprueba la **matriz de acceso actor × operación** (sustituye a la antigua matriz «rol × tenant», que presumía tenencia por colegio); (3) autorización explícita y separada de Brent. **No depende de W-B2d.** Su título, gate y notas dejan de describir aislamiento por colegio/tenant y pasan a plantillas globales, gestión solo-admin literal y consumo por asignación.
- **`W-PC-06`** conserva `DONE`/`AUTHORIZED`/clase 0 y todos sus campos; sus `notes` ganan la supersesión semántica fechada con la conclusión efectiva A.
- El mapa reclamación↔trabajo **no cambia** (152 enlaces, 59 P0): `W-B2d-01` conserva su enlace histórico único y `W-B2c-01` sus dos reclamaciones.

### 14.4 Alcance corregido de B2c — inventario de solo lectura del repositorio (2026-08-29, base `72f7beed`)

El inventario anterior — «dos tablas y seis funciones» — era **incompleto** y no se congela como alcance. El inventario nuevo, verificado archivo por archivo y registrado en detalle en `docs/reviews/w-b2c-01-learning-path-governance-correction-2026-08-29.md`, comprende: **cuatro tablas** (`learning_paths` y `learning_path_courses` sin RLS y con GRANT ALL a `anon`/`authenticated`; `learning_path_assignments` con RLS activa pero **cuatro políticas permisivas `USING (true)` sin cláusula `TO`** — aplican a todos los roles, `anon` incluido — más una política propia de progreso; `learning_path_progress_sessions` con políticas de fila propia razonables); **ocho funciones SECURITY DEFINER** con EXECUTE a `anon`/`authenticated`/`service_role` — las seis históricas **más `increment_path_assignment_time` y `update_session_heartbeat`**, ausentes del inventario anterior pese a mutar la misma superficie de progreso —, varias de las cuales aceptan **identificadores de actor suministrados por el caller** (`p_created_by`, `p_updated_by`, `p_assigned_by`, `p_user_id`) como entrada de autorización; el **efecto colateral de inscripción**: asignar auto-inserta filas en `course_enrollments` y desasignar **no** las elimina; la vía alterna de acceso `courses_learning_path_member_view` sobre `courses` (correctamente derivada de `auth.uid()`); las relaciones de resumen **inexistentes en migraciones** (solo el seeder no versionado — `W-B6d-01`) y **cinco referencias a RPC inexistentes** en el código; **15 rutas API** de rutas de aprendizaje (con `lib/services/learningPathsService.ts.hasManagePermission` concediendo gestión a `admin`, `equipo_directivo` y `consultor` — la sobre-concesión que la decisión 7 rechaza; conteo corregido de 16 a 15 en la ronda 1 de revisión, 2026-08-30, con la lista intacta) **más la ruta adyacente `pages/api/admin/users.ts`** — GET de administración de usuarios autorizado a `admin` y `equipo_directivo` vía `checkIsAdminOrEquipoDirectivo`, con **cliente service-role** que lee asignaciones directas y de grupo de `learning_path_assignments` unidas a `learning_paths`; registrarla **no autoriza** lecturas de reportes para roles no-admin — lo decide la matriz de acceso actor × operación de Privacidad (añadida al inventario y al check 22 en la ronda 1) —, **2 rutas de mantenimiento** (`update-learning-path-summaries` con cliente service-role **sin guardia de autenticación alguna**; `cleanup-learning-path-sessions` con verificación de `CRON_SECRET` que **queda abierta si la variable no está definida**; ninguna agendada en `vercel.json`), **4 rutas de matriz de asignación**, y **lecturas directas desde el navegador** a las tablas en tres puntos del código. `middleware.ts` restringe la UI `/admin/*` a `admin`, así que la brecha real está en las fronteras de API y base de datos.

### 14.5 Efecto sobre el validador

- `STATUSES` gana **`SUPERSEDED`** (aditivo).
- Los conjuntos aprobados del check **[18 alcance]** para `W-B2c-01` pasan a **4 tablas + 8 funciones** (disjunción B2b/B2c/B10a ahora 14 + 4 + 6 = 24 nombres únicos); el check **[19 funciones]** verifica en consecuencia que las ocho existen `SECURITY DEFINER` en el baseline.
- El check **[21 clasificación]** se revisa **deliberadamente** — tal como su propio comentario exigía para un cambio de estado legítimo: conserva todos los anclajes históricos (clasificación B literal, inventario de consultas, divulgación de la Management API, cierre PR #61) y sustituye los pins de estado prospectivos: `W-B2d-01` debe estar `SUPERSEDED`/`UNAUTHORIZED` (nunca `DONE`), `W-B2c-01` debe estar `BLOCKED` con clase `2`, y la dependencia ordenada B2d→B2c queda reemplazada por su prohibición.
...
| Work items | 107 | **107** |
| Enlaces / enlaces P0 | 152 / 59 | **152 / 59** |
| Lotes de fusión / work items en lotes | 28 / 34 | **28 / 34** |
| Enlaces P0 dentro / fuera de lotes | 39 / 20 | **39 / 20** |
| `MERGE`/`DATA`/`PRODUCTION_CHECK`/`DOCUMENTATION` | 90/10/6/1 | **90/10/6/1** |
