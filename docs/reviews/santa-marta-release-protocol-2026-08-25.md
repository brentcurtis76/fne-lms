...
**Revisión vigente:** **revisión 11** (2026-09-03, enmienda documental `SM-SIM-PROD-D1` autorizada por Brent, todavía local, no commiteada ni mergeada; la primera revisión independiente solicitó fijar el ref exacto y esa remediación está aplicada localmente). La decisión del dueño es no crear staging y reutilizar únicamente el proyecto Supabase de Producción con ref inmutable `sxlogxqzmarhqsblxmtj` y sus tenants sintéticos existentes: `QA Test School` (257) como base y `QA School B — Liceo de Prueba` (259) como control. `W-SIM-01` y `W-SIM-02` conservan sus formas, estados `BLOCKED`/`UNAUTHORIZED`, autoridad separada y cuatro enlaces **NO CERRANTE**, pero cambia su topología: W-SIM-01 debe implementar primero contención por tenant para etiqueta/reportes/correo/Zoom/cron y tooling de brechas; W-SIM-02 queda como ejecución clase 3 posterior y nominal sobre únicamente 257/259. La etiqueta vigente es `QA INTERNO — DATOS SINTÉTICOS EN PRODUCCIÓN — NO ES PILOTO REAL`. Esta revisión documenta cuatro consultas correctas de solo lectura y agregados a Producción solicitadas por Brent; no autoriza código, commit, push, PR, merge, deploy, clasificación, seed/reset, correo, Zoom ni otra escritura de Producción. Totales y mappings no cambian respecto de revisión 10.

**Historial de revisiones:**

**Fecha:** 25 de agosto de 2026 · **revisión 10** (2026-09-02, `SM-SIM-D0` documental autorizado por Brent: se reconcilia el merge ya ocurrido de la gobernanza de rutas — PR #65, head aprobado `d8f9ea38`, merge `49814091`, CI del PR `33397850894`, CI post-merge `33400056341` y despliegue automático Production `6182645350` en éxito —, con lo que el prerrequisito 1 de `W-B2c-01` queda cumplido; B2c sigue `BLOCKED` y sin autorizar por sus dos compuertas restantes — aprobación de Privacidad de la matriz actor × operación y autorización explícita separada de Brent. Se incorpora el plan completo de simulación sembrada y se crean `W-SIM-01` — `MERGE`, clase 0, lote `SIM1`, rama `feat/sm-sim` — y `W-SIM-02` — `DATA`, clase 3 — ambos `BLOCKED`/`UNAUTHORIZED`; sus cuatro enlaces a `SWEEP-MI-APRENDIZAJE-09` y `SWEEP-ONBOARDING-DATA-01` son evidencia **NO CERRANTE**, no remediación y no cambian el estado de ninguna reclamación. Totales: 160 reclamaciones / 36 P0 · 109 work items · 156 enlaces · 63 enlaces P0 · 29 lotes con 35 work items; modos 91/11/6/1; estados 29/57/19/3/1. El check [23 simulación] fija esta separación y los hashes congelados permanecen intactos. Nada de esta revisión aprovisiona, implementa, siembra, despliega, accede a producción ni autoriza W-SIM-01, W-SIM-02 o B2c) · *revisión 9* (2026-08-29, corrección de semántica global de rutas de aprendizaje por decisión del dueño — Brent, 2026-08-29: las `learning_paths` son **plantillas globales FNE** — ningún colegio es dueño de una ruta, no son específicas de generación, y `school_id`/`generation_id` NULOS son **alcance global intencional**, no propiedad ausente o corrupta; la asignación por la matriz afecta disponibilidad y uso, no crea propiedad; la gestión — crear, editar, borrar, asignar, desasignar — es **exclusiva del rol literal `admin`** y ningún otro rol la tiene, incluidos `equipo_directivo` y `consultor`; el usuario asignado consume y actualiza solo su propio progreso permitido. La clasificación **B** histórica de `W-PC-06` (2026-08-28) se conserva íntegra como registro correcto de su inferencia, y su **conclusión efectiva pasa a ser la clasificación A** — ninguna transformación de datos de rutas existentes es necesaria (registro de evidencia §8). **`W-B2d-01` queda `SUPERSEDED`** — retirado sin ejecutar: el backfill propuesto no se diseñó, no se autorizó, no se ejecutó y no se representa como completado, y **deja de ser prerrequisito de B2c**; `SUPERSEDED` es el valor de estado añadido como representación mínima coherente de «retirado sin ejecutar por rechazo de premisa». **`W-B2c-01` se reclasifica a clase 2** (corrección de seguridad/RLS sobre plantillas globales y consumo por asignación), sigue **`BLOCKED` y sin autorizar para implementación**, con exactamente tres prerrequisitos: (1) esta corrección de gobernanza aprobada de forma independiente y mergeada; (2) Privacidad aprueba la matriz de acceso actor × operación; (3) autorización explícita y separada de Brent. Su alcance queda congelado recién ahora, tras el inventario de solo lectura del repositorio: **cuatro tablas y ocho funciones SECURITY DEFINER** — no las «dos tablas y seis funciones» del inventario anterior — más las rutas API, servicios, clientes de navegador y efectos colaterales registrados en `docs/reviews/w-b2c-01-learning-path-governance-correction-2026-08-29.md`. Los estados pasan a `SCHEDULED` 29 · `BACKLOG` 57 · `BLOCKED` 17 · `DONE` 3 · `SUPERSEDED` 1; los totales 107 work items · 152 enlaces · 59 enlaces P0 · 28 lotes no cambian; el validador gana el estado `SUPERSEDED`, la revisión deliberada del check [21 clasificación] y el check nuevo **[22 semántica global]**, que ancla además el SHA-256 de la instantánea congelada de reclamaciones; la instantánea congelada y el legacy archivado no se tocan) · *revisión 8* (2026-08-28, cierre de W-PC-06 y creación de B2d: la comprobación de clasificación de datos de rutas de aprendizaje se ejecutó en solo lectura con autorización explícita de Brent — Brent corrió las consultas 1-4 en el SQL Editor de producción y Codex la consulta 5 a través del CLI ya vinculado — la única llamada de consulta de base de datos por la Management API explícitamente autorizada, un único SELECT acotado de solo lectura —, únicamente tras una autorización explícita posterior de Brent, también de solo lectura; solo se devolvieron agregados y esquema, sin filas ni PII — con resultado **B — DATA TRANSFORMATION REQUIRED**; `W-PC-06` pasa a `DONE`/`AUTHORIZED`/clase 0 con su registro agregado en `docs/reviews/w-pc-06-learning-path-data-classification-2026-08-28.md`; la clasificación crea la reparación gobernada de clase 3 **W-B2d-01** (lote `B2d`, rama `data/lp-scope`, `BLOCKED`/`UNAUTHORIZED`; sin elegir colegios ni semántica de propiedad, sin algoritmo y sin SQL), que debe autorizarse por separado, implementarse, revisarse de forma independiente, mergearse y ejecutarse de forma segura ANTES de programar B2c — B2c sigue `BLOCKED` y no absorbe ese trabajo; el registro de cierre de B2b mergeado como PR #60 (`7a470fe8`, CI post-merge 33183489941 en verde, despliegue automático completado) queda anclado y W-B2b-01 no se reabre ni su migración `20260827170000` se re-aplica; los totales pasan a 107 work items · 152 enlaces · 59 enlaces P0 · 28 lotes; la instantánea congelada no se toca) · *revisión 7: cierre técnico en producción de W-B2b (2026-08-28) — implementación en fix/rls-anon aprobada sin hallazgos por revisión independiente y mergeada como PR #59 (merge 0377edbf), migración 20260827170000 aplicada por Brent en el SQL Editor de producción en una sola transacción con su fila de ledger, preflight de once y postflight de ocho aserciones todos true y los catorce conteos de filas intactos; aceptación de Privacidad de W-B2b-01 evidenciada con persona y fecha en el comentario gobernado del PR #59; W-B2a y W-B2b técnicamente DONE y el inventario de ramas vigente suma fix/rls-anon* · *revisión 6: corrección de gobernanza aprobada por Brent (2026-08-27) — división del antiguo W-B2b-01 en B2b de catorce tablas y la frontera B2c bloqueada con W-PC-06 sin autorizar, B10a de ocho a seis, cierre en producción de B2a, más las remediaciones de las dos revisiones independientes (funciones B2c exactas del baseline, semántica de aceptación de dos registros, inventario de ramas verificado, unidades diferidas D-RLS-01/02/03 en §9; detalle en §12 del informe de normalización)* · *revisión 5: ronda 2 de normalización — corrigió los conteos de estado y verificación que la revisión 4 tenía mal, separó B1c y B6c, fusionó tres solapamientos y endureció el validador*
...
<!-- LEDGER-SUMMARY:BEGIN -->
{
  "total_claims": 160,
  "unique_p0_claims": 36,
  "total_work_items": 109,
  "total_claim_work_links": 156,
  "p0_claim_work_links": 63,
  "unique_p0_claims_with_links": 36,
  "merge_batches": ["B1a","B1b","B1c","B2a","B2b","B2c","B2d","B3a","B3b","B3c","B4a","B4b","B4c","B4d","B5","B6a","B6b","B6c","B6d","B7a","B7b","B8a","B8b","B8c","B9a","B10a","B10b","B10c","SIM1"]
}
<!-- LEDGER-SUMMARY:END -->
...
| Work items | **109** |
| Enlaces reclamación↔trabajo | **156** |
| Enlaces P0 reclamación↔trabajo | **63** |
...
De los 91 `MERGE`, **35 están dentro de los 29 lotes de fusión** y 56 no tienen lote: son defectos reales sin programar. El único `SUPERSEDED` es `W-B2d-01` — retirado **sin ejecutar** el 2026-08-29 porque la decisión del dueño rechazó su premisa de propiedad por colegio (§3, §9); no es un cierre, no ejecutó nada y no cierra ningún lote. Los `DONE` son tres. El tercero es `W-PC-06` — la comprobación de clasificación de datos en solo lectura, ejecutada el 2026-08-28 con autorización explícita de Brent y cerrada con clasificación **B — DATA TRANSFORMATION REQUIRED** (§9; su semántica quedó superseded el 2026-08-29 por decisión del dueño y la **conclusión efectiva es la clasificación A** — §9): es una dependencia de evidencia, no una remediación, y su `DONE` no cierra ningún lote. Los otros dos son `W-B2a-01` y `W-B2b-01` — ambos **técnicamente cerrados en producción** (B2a el 2026-08-27: PR #56, merge `0a6576c9`, verificación de solo lectura de Brent; B2b el 2026-08-28: PR #59, merge `0377edbf`, migración `20260827170000` aplicada por Brent en una sola transacción y verificada con preflight/postflight completos). `DONE` registra el cierre técnico; **la aceptación de la parte interesada se lleva aparte (§2)** y difiere entre ambos: para `W-B2a-01` sigue **pendiente de registro con persona y fecha**, mientras que para `W-B2b-01` la aceptación exigida por su gate (firma Privacidad) **está evidenciada** en un registro mutable gobernado — Brent Curtis, Project Lead y aprobador de Privacidad designado para W-B2b-01, 2026-08-27, comentario de aprobación del PR #59. Ninguno de los dos hechos puebla ni modifica la instantánea congelada de reclamaciones.
...
**Un lote = una rama = una fusión.** Un lote es un **contenedor de entrega**; un work item es una **unidad de remediación**. No son lo mismo: varios work items pueden compartir lote y rama. Los 29 lotes de §4 y sus 35 work items están **reconciliados contra los ledgers por `scripts/check-ledger.mjs`**, no derivados de esta prosa.
...
- **B2c** (la corrección de seguridad/RLS de rutas de aprendizaje, separada de B2b en la revisión 6) es **clase 2** desde la revisión 9: la pregunta que `W-PC-06` existía para responder quedó respondida — la clasificación **B** histórica (2026-08-28) se conserva como registro, y la **conclusión efectiva es la clasificación A** por la decisión del dueño del 2026-08-29 (plantillas globales FNE; los NULOS son alcance global intencional), así que **ningún trabajo de clase 3 precede a B2c**. `W-B2d-01` quedó **`SUPERSEDED` sin ejecutar** — su premisa de propiedad por colegio fue rechazada; el backfill no se diseñó, no se autorizó, no se ejecutó y no se representa como completado — y **no es prerrequisito de B2c**. `W-B2c-01` permanece **`BLOCKED` y sin autorizar para implementación**, con exactamente tres prerrequisitos, en orden: (1) la corrección de gobernanza del 2026-08-29 **aprobada de forma independiente y mergeada**; (2) **Privacidad aprueba la matriz de acceso actor × operación** (sustituye a la antigua matriz «rol × tenant», que presumía tenencia por colegio); (3) **autorización explícita y separada de Brent**. Su alcance corregido — cuatro tablas, ocho funciones SECURITY DEFINER, rutas API, servicios, clientes de navegador y efectos colaterales — y sus criterios de aceptación viven en `docs/reviews/w-b2c-01-learning-path-governance-correction-2026-08-29.md`; documentar esto **no autoriza ninguna consulta ni escritura a producción**.
...
## 4. Los 29 lotes atómicos

La columna **P0 (enlaces)** cuenta **enlaces reclamación↔trabajo de severidad P0**, no reclamaciones P0. Los 29 lotes contienen **35 work items** y suman **41** de los **63** enlaces P0; los otros 22 caen en el carril de datos (15) y en las comprobaciones de producción (7). Los cuatro enlaces de `W-SIM-01`/`W-SIM-02` son evidencia NO CERRANTE y no remediación: dos quedan dentro del lote `SIM1` y dos en el ítem `DATA` sin lote.
...
| **B2c** | `fix/rls-learn` | **2** | 1 | 2 | Privacidad | *(corrección de seguridad/RLS de plantillas globales, gestión solo del rol literal admin; sigue BLOCKED — prerrequisitos: corrección de gobernanza 2026-08-29 aprobada y mergeada, matriz de acceso actor × operación aprobada por Privacidad, autorización explícita y separada de Brent)* |
| **B2d** | `data/lp-scope` | **3** | 1 | 1 | Privacidad | *(SUPERSEDED 2026-08-29, sin ejecutar — la decisión del dueño rechazó su premisa de propiedad por colegio; no precede a B2c; la fila queda como registro histórico del lote y no hay backfill pendiente ni autorizado)* |
...
| **SIM1** | `feat/sm-sim` | 0 | 1 | 2 | Brent | Guardas, aislamiento o determinismo no prueban el contrato; su merge NO autoriza datos alojados |
...
| Piloto de un colegio, sólo personal | B1a–c · B2a–**c** · B3a–c · B4a–d *(B2d: `SUPERSEDED`, ya no se exige)* | Una directora + su comunidad + su consultor |
...
**El piloto de personal exige B2c además de B2b.** Las veintidós tablas del hallazgo histórico se remedian ahora en tres unidades: B2b cierra las **14 sin referencias**, B2c cierra la **corrección de seguridad/RLS de rutas de aprendizaje** — cuyo alcance corregido en la revisión 9 es de **cuatro tablas** (`learning_paths`, `learning_path_courses`, `learning_path_assignments`, `learning_path_progress_sessions`) **y ocho funciones SECURITY DEFINER**, más sus rutas API, servicios y efectos colaterales (inventario completo en `docs/reviews/w-b2c-01-learning-path-governance-correction-2026-08-29.md`) — y B10a diseña política para las **6 referenciadas restantes**. Desde la revisión 9, **B2d ya no existe como paso previo**: `W-B2d-01` quedó `SUPERSEDED` sin ejecutar porque la decisión del dueño estableció que las rutas son plantillas globales FNE; B2c espera sus **tres prerrequisitos propios** (§3): corrección de gobernanza mergeada, matriz de acceso actor × operación aprobada por Privacidad, y autorización explícita y separada de Brent. Mientras B2c no cierre, las tablas de rutas de aprendizaje siguen alcanzables de forma anónima, así que cerrar solo B2b **no** deja el piloto en un estado seguro.

**Dos ejes en RLS.** Audiencia: personal antes que estudiantes. Riesgo de regresión dentro de cada nivel: grupo A (B2b, exactamente 14 tablas sin referencias) primero — ya cerrado —; la corrección de rutas de aprendizaje (B2c, hoy `BLOCKED` tras sus tres prerrequisitos de la revisión 9) después, y grupo B (B10a, las 6 referenciadas restantes) al final; la antigua etapa intermedia de reparación de propiedad (B2d) quedó `SUPERSEDED` sin ejecutar y desaparece de la secuencia. El hallazgo original de 14 + 8 tablas sigue siendo históricamente correcto: las ocho referenciadas son hoy dos (B2c en su núcleo histórico) más seis (B10a), y el alcance vigente de B2c añade las dos tablas de asignaciones y sesiones de progreso que el inventario de 2026-08-29 incorporó.
...
| 6 | Clasificación de datos de rutas de aprendizaje *(decidió el backfill y la clase de B2c — **ejecutada 2026-08-28, clasificación B**; semántica superseded 2026-08-29: **conclusión efectiva clasificación A**, sin backfill)* | `W-PC-06` |
...
**`W-PC-06` — ejecutada y cerrada (2026-08-28).** Cronología de autorización: Brent (`authorization_owner`) autorizó explícitamente la comprobación en solo lectura y **ejecutó personalmente las consultas 1-4 en el SQL Editor de producción**; antes de la consulta 5, **Codex hizo una única lectura de metadatos por la Management API, de solo lectura (`supabase projects list --output json`), para confirmar el enlace ya existente y su estado** — sin consultar filas, sin PII y sin crear ni cambiar nada; **no es una de las cinco consultas de base de datos**; la **consulta 5 la ejecutó Codex a través del CLI ya vinculado (`supabase db query --linked`, que también opera por la Management API de Supabase), únicamente tras una autorización explícita posterior de Brent, también de solo lectura** — fue la **única llamada de consulta de base de datos por la Management API explícitamente autorizada** y envió **un único SELECT acotado de solo lectura**; **ninguna de las dos llamadas por la Management API creó ni cambió enlace de Supabase, credencial, configuración del proyecto, dato, esquema, GRANT, política, ajuste de RLS, función, ledger de migraciones, despliegue ni ningún otro estado**. Evidencia devuelta — **solo agregados y esquema; ninguna fila, identificador, nombre, correo, texto libre ni PII de menores**: 7 `learning_paths`, las 7 con `school_id` y `generation_id` nulos y **ninguna con colegio efectivo resoluble**; 6 con `created_by` y 1 sin él, y cada uno de los 6 creadores tiene exactamente un candidato de colegio por rol activo con alcance **pero también un rol activo sin alcance — no es propiedad autoritativa**; 22 `learning_path_courses`, cero huérfanas, las 22 heredan el alcance padre sin resolver; **883 asignaciones, todas directas a usuario y cero de grupo** — 86 con cero candidatos de colegio, 793 con exactamente uno y 4 con varios; de los 7 paths, **los 6 con asignaciones abarcan todos varios colegios candidatos** y 1 no tiene asignaciones. Resultado: **clasificación B — DATA TRANSFORMATION REQUIRED**, registrada con fecha y personas en el work item y, en detalle, en `docs/reviews/w-pc-06-learning-path-data-classification-2026-08-28.md`. Consecuencia gobernada en su fecha: la reparación de clase 3 era `W-B2d-01` (§3, §4), que **no eligió colegios, no decidió la semántica global/compartida frente a por-colegio, no definió algoritmo y no escribió SQL** — la decisión de semántica quedaba abierta y era de Brent. Este cierre no autoriza ninguna otra consulta a producción.

**`W-PC-06` — supersesión semántica (2026-08-29).** Brent tomó la decisión de semántica que el cierre anterior dejó explícitamente abierta: las `learning_paths` son **plantillas globales FNE** — ningún colegio es dueño de una ruta, no son específicas de generación, y los `school_id`/`generation_id` NULOS observados por las consultas 2 y 3 son **alcance global intencional**, no propiedad ausente o corrupta; la asignación (consulta 5) afecta disponibilidad y uso sin crear propiedad, así que la dispersión multi-colegio de los paths asignados es el comportamiento esperado de una plantilla global, no una anomalía. La cronología, las cinco consultas, la lectura de metadatos y todos los agregados anteriores **se conservan intactos y correctos**; lo que cambia es la inferencia: la premisa «cada ruta necesita un colegio dueño derivable» fue rechazada por el dueño, y con ella cae la clasificación B como conclusión vigente. **Conclusión efectiva: clasificación A — ninguna transformación de datos de rutas de aprendizaje existentes es necesaria.** `W-B2d-01` queda `SUPERSEDED` sin ejecutar (ningún backfill se diseñó, autorizó, ejecutó ni se representa como completado) y deja de ser prerrequisito de B2c; `W-B2c-01` pasa a clase 2 y espera sus tres prerrequisitos de §3. La gestión de rutas — crear, editar, borrar, asignar, desasignar — es **exclusiva del rol literal `admin`**; ningún otro rol la tiene, incluidos `equipo_directivo` y `consultor`, hoy sobreprivilegiados en la aplicación y en las funciones de base de datos. Registro completo de la decisión, del modelo de acceso, del inventario corregido de superficie y de los criterios de aceptación futuros: `docs/reviews/w-b2c-01-learning-path-governance-correction-2026-08-29.md`, y §8 del registro de evidencia. Esta supersesión **no autoriza** implementación alguna ni ninguna consulta a producción.
...
## 10. Simulación sembrada Santa Marta — revisión 10 (histórica y supersedida en topología)

El siguiente ensayo de Santa Marta será una **simulación sintética en staging**, no un piloto real. No involucra personas ni datos reales de Santa Marta, no demuestra onboarding real, adopción real, aceptación institucional ni preparación de producción, y toda evidencia debe llevar la etiqueta visible `PILOTO SIMULADO — DATOS SINTÉTICOS — NO PRODUCCIÓN`.

- `W-SIM-01` (`MERGE`, clase 0, lote `SIM1`, rama planificada `feat/sm-sim`) gobierna la futura implementación de guardas de destino, aislamiento de correo, banner/noindex, `ZOOM_MODE=mock`, seeder/reset/verificador deterministas y sus pruebas. Está `BLOCKED` y `UNAUTHORIZED`.
- `W-SIM-02` (`DATA`, clase 3, sin lote) gobierna la futura inicialización del proyecto staging allowlisted y la ejecución de `sm-sim-v1`. Está `BLOCKED` y `UNAUTHORIZED`; exige autorización separada incluso después de que `W-SIM-01` se implemente, revise y fusione.
- Cada ítem se enlaza a `SWEEP-MI-APRENDIZAJE-09` y `SWEEP-ONBOARDING-DATA-01` únicamente como evidencia **NO CERRANTE**. Los cuatro enlaces no son remediación, no cambian el estado de las reclamaciones y no cuentan en su cobertura de remediación.

La secuencia gobernada es: **SM-SIM-D0** (esta documentación) → designación externa y separada del proyecto Supabase no productivo, entorno Vercel staging, dominio protegido y sumidero de correo → **W-SIM-01** → revisión independiente y merge → autorización separada de **W-SIM-02** → inicialización verificada → siembra/aceptación/reset. Fusionar D0 no autoriza aprovisionar recursos, implementar, desplegar, consultar o escribir una base alojada. Fusionar W-SIM-01 tampoco autoriza W-SIM-02.

La corrección de gobernanza de rutas de aprendizaje de PR #65 (`d8f9ea38` → merge `49814091`) satisface solamente el prerrequisito 1 de `W-B2c-01`. B2c permanece `BLOCKED` y sin autorizar hasta que Privacidad apruebe la matriz actor × operación y Brent autorice explícitamente la implementación. Mientras siga abierto, la simulación no se comparte de forma amplia ni pública.

La instantánea congelada de reclamaciones y el ledger legacy archivado permanecen byte a byte intactos. Esta revisión sólo añade planificación mutable y comprobaciones del ledger; no accedió a Supabase, Vercel ni producción y no efectuó ninguna operación de base de datos.

## 11. Simulación con QA sintético en Producción — revisión 11

Por decisión explícita de Brent del 2026-09-03, **no se crea staging**. La simulación reutiliza únicamente `QA Test School` (257) como base sintética y `QA School B — Liceo de Prueba` (259) como control entre colegios, dentro del proyecto Supabase de Producción. La evidencia y toda superficie exclusiva de esos tenants usa la etiqueta exacta `QA INTERNO — DATOS SINTÉTICOS EN PRODUCCIÓN — NO ES PILOTO REAL`.

La inspección solicitada por Brent observó, mediante cuatro consultas correctas de solo lectura y agregados, que 257 ya contiene cuentas QA aprobadas, siete tipos de rol activos, comunidad, sesiones, contrato y ledger de horas, estructura de cursos, matrículas de curso y estados parciales de evaluación; 259 permanece deliberadamente escaso. No se recuperaron datos personales ni filas de usuario. La inspección también confirmó brechas: red, generaciones, inscripciones de programa, licitaciones, asignaciones/progreso de rutas, submissions de evaluación, transformación y asistencia Zoom. Ambos colegios siguen clasificados `client`; esa clasificación y la falta de fronteras por tenant impiden cualquier siembra hoy.

- `W-SIM-01` (`MERGE`, clase 0, lote `SIM1`, rama planificada `feat/sm-sim`) permanece `BLOCKED`/`UNAUTHORIZED`. Debe implementar primero, sin migraciones ni acceso alojado, la allowlist exacta de ref+schools 257/259; requisito `tenant_kind=qa`; etiqueta y exclusión de reportes por tenant; correo `suppressed_qa` antes del proveedor; Zoom falso/rechazado antes del proveedor sin modo global; cobertura de cron; y tooling determinista de brechas cuyo reset nunca toque filas preexistentes o sin propiedad.
- `W-SIM-02` (`DATA`, clase 3, sin lote) permanece `BLOCKED`/`UNAUTHORIZED`. Después de que W-SIM-01 sea aprobado, mergeado, desplegado y verificado en el SHA exacto, y sólo con otra autorización nominal de Brent, puede clasificar mediante escritor privilegiado únicamente 257/259 como `qa` y sembrar únicamente brechas propiedad del manifiesto. Volverlos a `client` no es una compensación automática: requiere otra autorización y prueba de que no quedan residuos sintéticos capaces de disparar comportamiento real.
- Las rutas de aprendizaje son plantillas globales FNE: la simulación no crea plantillas globales en Producción. Su asignación/progreso queda diferida mientras W-B2c siga abierto, salvo una autorización posterior exacta que respete su frontera ya aprobada.
- Los cuatro mappings existentes de W-SIM-01/02 permanecen **NO CERRANTE** y no son remediación. La topología cambió; los conteos del ledger no.
