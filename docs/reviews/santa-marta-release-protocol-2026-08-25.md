...
**La propiedad vive en los work items, nunca en las reclamaciones.** `autoridad_aceptacion` es un **rol**. Y desde la revisión 6 el protocolo distingue **dos registros que nunca se confunden**: el **registro congelado de reclamaciones** es una instantánea histórica preservada byte a byte — sus campos `evidencia_prod`, `firmado_por` y `fecha_firma` quedaron vacíos en las 160 filas al congelarse y **permanecen así para siempre**; rellenarlos exigiría editar el archivo congelado y romper su preservación. La **evidencia técnica de cierre y la aceptación** son mutables y se registran donde corresponde: en el work item (`notes`/`gate_salida`, como hace `W-B2a-01`) o en un acta de aceptación gobernada aparte. El vacío del registro congelado documenta honestamente lo que faltaba **en el momento de congelar**; el trabajo restante se mide contra el registro mutable, no rellenando la instantánea.
...
**Un lote no está entregado hasta que su evidencia técnica de cierre y su aceptación con persona y fecha constan en el registro mutable** — el work item (`notes`/`gate_salida`) o un acta de aceptación gobernada que el lote referencie, como el cierre de `W-B2a-01`. Los campos `evidencia_prod`/`firmado_por`/`fecha_firma` del **registro congelado** de reclamaciones son una instantánea histórica y **no se rellenan nunca** (revisión 6; editar el archivo congelado rompería su preservación byte a byte).
...
**Inventario de ramas vigente (verificado contra `git ls-remote` el 2026-08-27).** De estas 27 ramas de lote, sólo **tres existen** en el repositorio canónico y en el remoto vivo: `fix/horas-rep` (`f6d0e908`), `fix/gate-score` (`63616d61`) y `fix/red-super` (`63fc8c9c`; B2a, ya mergeada vía PR #56). **`fix/auth-sec2` — la base planificada de `auth/rebase-z7` (B10c) — está AUSENTE del repositorio canónico y del remoto vivo**: sobrevive únicamente como **copia de seguridad congelada** en el commit `4b87243c`, sin rama ni ref que lo apunte; integrarla exige primero restaurarla desde esa copia, con decisión explícita de Brent. Las otras 24 ramas son **identificadores de lote planificados, sin commits detrás** — `fix/rls-learn` (B2c) y `fix/rls-anon` (B2b) incluidas. La rama remota de investigación `fix/rls-public` (`565faa0d`) **no** es la rama de ningún lote: queda aparcada como evidencia de solo lectura y sus commits no se reutilizan en bloque.
...
|---|---|---|
| **D-RLS-01** | **Re-basar sobre `main` actual la superficie EXECUTE de funciones no-learning:** `has_transformation_access`, `get_available_assignment_templates`, `cleanup_propuesta_rate_limits`, `has_global_workspace_access`, `submit_quiz`. Es redescubrimiento fresco —callers, políticas dependientes y privilegios— **antes de cualquier implementación**. `profiles_role_backup` queda excluida: el lockdown de su tabla ya lo gobierna B2b. | DIFERIDA; ninguna implementación definida |
...
| **D-RLS-03** | **Re-basar y clasificar la superficie `SECURITY DEFINER` restante con GRANT a `anon`.** La cifra «80 firmas» del plan antiguo es **evidencia histórica medida en el head de investigación `565faa0d`**, no un conteo vigente: antes de programar nada se exige un **reconteo local sobre `main` actual**. D-RLS-03 es **sólo descubrimiento**; la remediación posterior queda indefinida hasta que exista esa evidencia. | DIFERIDA; reconteo previo obligatorio |
...
**La aceptación pendiente se mide en el registro mutable, no en la instantánea congelada.** Las 160 filas del registro congelado conservan `firmado_por` y `fecha_firma` vacías **por diseño y para siempre** (son una instantánea histórica byte-preservada). El trabajo restante es registrar, lote por lote, la evidencia técnica de cierre y la aceptación con persona y fecha en los work items o en actas de aceptación gobernadas — exactamente como quedó registrado el cierre de `W-B2a-01`, el único hecho hasta ahora.
