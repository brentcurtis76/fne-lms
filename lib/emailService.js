...
const FROM_EMAIL = process.env.EMAIL_FROM_ADDRESS || 'notificaciones@fne-lms.com';
...
export class EmailNotificationService {
  ...
  async sendImmediateNotification(user, notification) {
    try {
      ...
      const result = await deliverOutboundEmail({
        authorization: await authorizeUserEmail(supabase, user.id),
        message: {
  ...
  async sendDigestEmail(user, notifications, digestType) {
    try {
      ...
      console.log('Digest email delivery result:', result);
      return result;
    } catch (error) {
