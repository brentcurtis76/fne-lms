 ...
 *
 * WHAT PROTECTS IT — reading first is not enough, because a read can fail while
 * the object is perfectly healthy, and a manual upload can land in the window
 * between our miss and our write. So the write is create-only (`upsert: false`):
 * the bucket decides who wins, and generated bytes can never replace a published
 * document. Losing that race is a success, not a failure — the object already
 * there stands, this request serves its own freshly generated copy, and the next
 * request reads what is stored. Create-only is what makes the path a publishing
 * mechanism instead of a cache with a convention attached to it.
 *
 ...
 *
 * The three rules interact: an owner-uploaded designed brochure is a manual
 * write to the same path, so it is served everywhere, unchanged by this gate —
 * only *generated* bytes are production-gated, and even in production they are
 * create-only.
 *
...
import { StorageObjectExistsError, downloadFile, uploadFile } from '../../propuestas/storage';
...
export function createPasantiasPdfHandler(route: PasantiasPdfRoute) {
  ...
  async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
    ...
    if (!pdf) {
      ...
      if (mayWriteCache()) {
        try {
          // Create-only: a generated document may fill an empty path, never
          // replace what is on one. See the manual-override note above — this
          // is the only thing standing between a transient read failure and
          // the owner's published brochure.
          await uploadFile(path, pdf, 'application/pdf', { upsert: false });
        } catch (err) {
          if (err instanceof StorageObjectExistsError) {
            // Someone got there first: a manual override published between our
            // read and our write, or another cold request that finished sooner.
            // Their object stands, ours was never written, nothing is wrong.
            console.info(
              `[pasantias-pdf] left the existing object at ${path} alone: generated bytes are ` +
                'uploaded create-only, so they never overwrite a manual override (D-05). Serving ' +
                'the freshly generated document for this request.'
            );
          } else {
