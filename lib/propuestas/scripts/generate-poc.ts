/**
 * Standalone PoC PDF generator.
 * Run with: npx tsx lib/propuestas/scripts/generate-poc.ts
 * Output: lib/propuestas/__tests__/poc-output.pdf
 */
...
async function main() {
  ...
  const doc = React.createElement(
    ...
    // Page 2: Dark section with body text
    React.createElement(
      DarkSection,
    ...
    // Page 3: Consultant cards on dark background
    React.createElement(
      Page,
      {
        size: 'A4',
  ...
  console.log(`✓ PDF written to ${outPath} (${(buffer.length / 1024).toFixed(1)} KB)`);
}
