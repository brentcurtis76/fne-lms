...
// Mock Supabase client via the wrapper that the service actually imports
...
describe('GroupAssignmentsV2Service', () => {
  ...
  describe('getGroupMembers - Data Structure', () => {
    ...
    it('should handle empty name fields gracefully', async () => {
      ...
      expect(result.members[0].user.full_name).toBe('Usuario desconocido');
    });
  ...
  describe('getOrCreateGroup - Policy Error Handling', () => {
    ...
    it('should return null group with no error when PGRST116 (no rows found)', async () => {
      setupSupabaseMock({
        group_assignment_members: {
          maybeSingleResult: {
            data: null,
      ...
      // getUserGroup treats PGRST116 as "no group yet" - returns null group, no error
      expect(result.group).toBeNull();
      expect(result.error).toBeNull();
    });
