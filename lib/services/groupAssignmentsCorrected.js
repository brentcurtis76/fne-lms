...
export const groupAssignmentService = {
  ...
  async getUnassignedStudents(assignmentId, communityId) {
    // Keep this list in sync with TEACHING_ELIGIBLE_ROLES in utils/roleUtils.ts.
    // Inlined here because this file is JS and the constant lives in a .ts module.
    const TEACHING_ELIGIBLE_ROLES = [
      'docente',
      'admin',
      'consultor',
      'equipo_directivo',
    ...
    // Find active teaching-eligible members of this community via user_roles,
    // then hydrate their profile data separately.
    ...
    const memberUserIds = Array.from(new Set((roleRows || []).map(r => r.user_id)));
    if (memberUserIds.length === 0) return [];

    // Require profiles.growth_community_id to agree with user_roles.community_id.
    // user_roles is authoritative for role assignment, but profiles still carries
    // community scoping. If they disagree (stale/drifted data), prefer the stricter
    // intersection so users aren't surfaced outside their actual community.
