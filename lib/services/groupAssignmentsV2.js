...
class GroupAssignmentsV2Service {
  ...
  async getGroupAssignmentsForUser(userId) {
    try {
      ...
      // Get learning paths for these courses
      let courseLearningPaths = {};
      try {
        const { data: lpCourses } = await supabase
          .from('learning_path_courses')
          .select(`
            course_id,
            learning_path:learning_paths(id, name)
          `)
          .in('course_id', courseIds);

        if (lpCourses) {
          lpCourses.forEach(lpc => {
            if (lpc.learning_path) {
              courseLearningPaths[lpc.course_id] = lpc.learning_path.name;
            }
          });
        }
      } catch (err) {
        ...
        lessonBlocks.forEach((block, blockIndex) => {
          if (block.type === 'group-assignment' || block.type === 'group_assignment') {
            groupAssignments.push({
              ...
              learning_path_title: courseLearningPaths[lesson.course.id] || null,
  ...
  /**
   * Get user's existing group for an assignment (does NOT auto-create)
   * Returns null if user is not in a group yet
   */
  async getUserGroup(assignmentId, userId) {
    try {
      // Check if user belongs to a group for this assignment
      ...
      if (memberCheckError) {
        ...
        console.error('[GroupAssignments] Error checking group membership:', memberCheckError);
        return { group: null, error: memberCheckError };
      }
  ...
  /**
   * DEPRECATED: Use getUserGroup() instead
   * This method auto-creates groups which doesn't match the student-choice model
   */
  async getOrCreateGroup(assignmentId, userId) {
    console.warn('[GroupAssignments] getOrCreateGroup is deprecated. Use getUserGroup() instead.');
    return this.getUserGroup(assignmentId, userId);
  }
