...
class ReportsService {
  ...
  async getUserProgress(userProfile, additionalFilters = {}) {
    try {
      ...
      // Step 3: Get course enrollment data (paginated to avoid URL length limits)
      let enrollments = [];
      let enrollmentError = null;
      
      // Process user IDs in batches of 50 to avoid URL length limits
      const batchSize = 50;
      for (let i = 0; i < filteredUserIds.length; i += batchSize) {
        const userBatch = filteredUserIds.slice(i, i + batchSize);
        ...
        if (batchError) {
          enrollmentError = batchError;
          break;
        }
        
        if (batchEnrollments) {
          enrollments.push(...batchEnrollments);
        }
      }
      ...
      // Step 4: Learning-path data — cross-user learning-path reporting is
      // literal-admin-only (W-B2c-01, closure C3). For any other audience no
      // learning-path query is issued and the learning-path time is reported
      // as unavailable (null), never 0. A failed query is surfaced, not
      // swallowed.
      let learningPathData = [];
      const learningPathAvailable = userProfile?.role === 'admin';
      if (learningPathAvailable) {
        for (let i = 0; i < filteredUserIds.length; i += batchSize) {
          const userBatch = filteredUserIds.slice(i, i + batchSize);
          const { data: pathData, error: pathError } = await supabase
            ...
            .in('user_id', userBatch);
          if (pathError) {
            console.error('Error fetching learning path summaries:', pathError);
            return { data: null, error: pathError };
          }
          if (pathData) {
            learningPathData.push(...pathData);
          }
        }
      }
  ...
  async getFilteredUserIds(roleFilters, additionalFilters) {
    try {
      let userIds = [];
      const { data: clientSchools, error: clientSchoolsError } = await supabase
        .from('schools')
        .select('id')
        .eq('tenant_kind', 'client');
      if (clientSchoolsError) throw clientSchoolsError;
      const clientSchoolIds = clientSchools?.map(s => s.id) || [];
      if (clientSchoolIds.length === 0) return [];
      ...
      if (roleFilters.consultant_id) {
        ...
        if (assignedIds.length > 0) {
          const { data: assignedProfiles } = await supabase
            ...
            .select('id')
            .in('id', assignedIds)
            .in('school_id', clientSchoolIds);
          userIds = assignedProfiles?.map(profile => profile.id) || [];
        }
      }
        ...
        const clientSchoolSet = new Set(clientSchoolIds);
        const schoolIds = networkSchools
          ?.map(ns => ns.school_id)
          .filter(schoolId => clientSchoolSet.has(schoolId)) || [];
   ...
   */
  aggregateUserProgress(profiles, enrollments, learningPathData, additionalFilters, learningPathAvailable = true) {
    ...
    const userProgressData = profiles.map(profile => {
      ...
      // Calculate time spent (from learning paths only, since course_enrollments doesn't have time_spent)
      const pathTimeSpent = userPathData.reduce((sum, p) => sum + (p.total_time_spent_minutes || 0), 0);
      const totalTimeSpentMinutes = pathTimeSpent;
      ...
      const userProgress = {
        ...
        // null = learning-path time unavailable to this audience (admin-only), never 0
        total_time_spent_minutes: learningPathAvailable ? Math.round(totalTimeSpentMinutes) : null,
  ...
  async getSummaryStats(userProfile) {
    try {
      ...
      if (users && users.length > 0) {
        ...
        summary.total_time_spent = users.every(u => u.total_time_spent_minutes === null)
          ? null
          : users.reduce((sum, u) => sum + (u.total_time_spent_minutes || 0), 0);
  ...
  async getAvailableFilters(userProfile) {
    try {
      ...
      if (userProfile.role === 'admin') {
        ...
        const clientSchoolIds = filters.schools.map(school => school.id);
        if (clientSchoolIds.length === 0) {
      ...
      // Other roles see filtered options based on their access
        ...
        if (roleFilters.school_id) {
          const { data: school } = await supabase
            ...
            .eq('tenant_kind', 'client')
            .single();
          if (!school) return { data: filters, error: null };
