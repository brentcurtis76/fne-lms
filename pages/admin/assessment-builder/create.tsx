...
import { AREA_LABELS, AREA_STATUS, AREA_CODES } from '@/types/assessment-builder';
...
const CreateTemplate: React.FC = () => {
  ...
  // Track auto-generated name pattern to detect user overrides
  const autoGeneratedNameRef = useRef('');
  ...
  // Auto-generate template name when area or grade changes
  useEffect(() => {
    if (!formData.area || !formData.grade_id) return;
    ...
    const fetchCountAndSetName = async () => {
      try {
        const params = new URLSearchParams({
          area: formData.area,
          grade_id: String(formData.grade_id),
          count_only: 'true',
        });
        ...
        const areaCode = AREA_CODES[formData.area as TransformationArea] || AREA_LABELS[formData.area as TransformationArea];
        const newName = `${areaCode}_${grade.name}_${year}_V${version}`;
        const prevAutoName = autoGeneratedNameRef.current;
        autoGeneratedNameRef.current = newName;

        // Only auto-populate if name is empty or matches a previously auto-generated name
        setFormData((prev) => {
        ...
        // Ignore fetch errors during auto-generation
      }
    };

    fetchCountAndSetName();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.area, formData.grade_id, grades]);
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              <label htmlFor="area" className="block text-sm font-medium text-gray-700 mb-1">
                Vía de Evolución <span className="text-red-500">*</span>
              </label>
              <select
                id="area"
                value={formData.area}
              ...
              <p className="mt-1 text-xs text-gray-500">
                Selecciona la vía de evolución para este template
              </p>
            </div>
            ...
            <div>
              ...
              {formData.name && formData.name === autoGeneratedNameRef.current && (
                <p className="mt-1 text-xs text-gray-500">
                  Nombre generado automáticamente. Puedes modificarlo.
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-brand_primary focus:border-brand_primary"
              />
              <p className="mt-1 text-xs text-gray-500">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="inline-flex items-center px-4 py-2 bg-brand_accent text-brand_primary rounded-lg shadow hover:bg-brand_accent/90 transition-colors text-sm font-medium disabled:opacity-50"
              >
                {isSaving ? (
                  <>
