/**
 * API endpoint for network management - TYPED VERSION
...
async function handleGetNetworks(supabase: any, res: NextApiResponse) {
  try {
    // Get networks with school counts - explicit columns for type safety
    const { data: networks, error } = await supabase
      ...
      `)
      .order('nombre')
      .returns<(RedesDeColegios & {
        red_escuelas: (Pick<RedEscuelas, 'school_id' | 'red_id' | 'fecha_agregada' | 'agregado_por'> & {
          schools: Pick<Schools, 'id' | 'name'> | null
        })[]
      })[]>();
    ...
    // Get supervisors for each network separately - with explicit types
    const networksWithStats = await Promise.all(
      (networks || []).map(async (network) => {
        ...
        const { data: supervisors } = await supabase
          ...
          .eq('is_active', true)
          .returns<(Pick<UserRoles, 'user_id' | 'created_at'> & {
            profiles: Pick<Profiles, 'id' | 'email' | 'first_name' | 'last_name'> | null
          })[]>();
        ...
        return {
          ...
          schools: network.red_escuelas?.map((re) => ({
            id: re.school_id as number, // Explicitly treat as number
            name: re.schools?.name,
            assigned_at: re.fecha_agregada,
            assigned_by: re.agregado_por
          })) || [],
          supervisors: supervisors?.map((ur) => ({
            user_id: ur.user_id,
...
async function handleCreateNetwork(
  ...
  try {
    ...
    // Create network - explicit insert type
    const { data: newNetwork, error } = await supabase
      ...
      } as Database['public']['Tables']['redes_de_colegios']['Insert'])
      .select('id, nombre, descripcion, created_by, last_updated_by, created_at, updated_at')
      .single()
      .returns<RedesDeColegios>();
...
async function handleUpdateNetwork(
  ...
  try {
    ...
    const updateData: Database['public']['Tables']['redes_de_colegios']['Update'] = {
      last_updated_by: adminId,
