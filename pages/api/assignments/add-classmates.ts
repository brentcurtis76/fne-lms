...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Ensure each classmate has at least one active role at the requester's school.
    // Extra active rows at other schools or with null school_id do not invalidate them.
    const missingIds = classmatesMissingSchool(
      classmateIds,
      classmateRoles ?? [],
      requesterSchoolId,
    );
    ...
    if (missingIds.length > 0) {
      console.error('[add-classmates] VALIDATION FAILED - School membership check');
      console.error('[add-classmates] Requested classmates:', classmateIds);
      console.error('[add-classmates] Missing at requester school:', missingIds);
      ...
      return res.status(400).json({
        ...
        details: { missingIds, requestedCount: classmateIds.length }
      });
    }
