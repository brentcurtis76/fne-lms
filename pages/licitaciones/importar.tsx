...
export default function ImportarLicitacionPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      const roles: Array<{ role_type: string; school_id?: number | null; school?: { id?: number; name?: string } | null }> =
  ...
  const validate = (): boolean => {
    ...
    if (!form.year) newErrors.year = 'Seleccione un ano';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.numero_licitacion.trim()) {
        body.numero_licitacion = form.numero_licitacion.trim();
      }
      ...
      if (!res.ok) {
        toast.error(json.error || 'Error al registrar licitacion historica');
        return;
      }
      ...
      toast.error('Error inesperado al registrar licitacion historica');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  // Year dropdown range: HISTORICO_MIN_YEAR .. currentYear
  const yearOptions = Array.from(
    { length: currentYear - HISTORICO_MIN_YEAR + 1 },
    (_, i) => currentYear - i,
  );
    ...
    >
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p className="text-gray-600 mb-8">
          Registre una licitacion pasada para mantenerla en el historial. Se guarda directamente como &quot;cerrada&quot;.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-lg shadow p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label="Ano" name="year" required errors={errors}>
                <select
                  id="field-year"
                  value={form.year}
            ...
            <Field label="Numero de Licitacion (opcional)" name="numero_licitacion" errors={errors}>
              <input
                id="field-numero_licitacion"
                type="text"
                value={form.numero_licitacion}
                onChange={e => setForm(f => ({ ...f, numero_licitacion: e.target.value }))}
                placeholder="Se genera automaticamente si se deja en blanco"
                className={inputClass}
                maxLength={100}
              />
            </Field>

            <Field label="Notas (opcional)" name="notas" errors={errors}>
              <textarea
                id="field-notas"
                value={form.notas}
                onChange={e => setForm(f => ({ ...f, notas: e.target.value }))}
                placeholder="Contexto adicional sobre esta licitacion historica..."
                rows={3}
                maxLength={2000}
                className={inputClass}
              />
          ...
          <div className="flex justify-end space-x-4">
            ...
            >
              {submitting ? 'Registrando...' : 'Registrar Licitacion Historica'}
            </button>
          </div>
        </form>
      </div>
