...
async function checkBrentPermissions() {
  console.log('🔍 Checking permissions for brent@perrotuertocm.cl\n');
  ...
  console.log('Roles:');
  roles.forEach(r => {
    console.log(`  ${r.role_type} (${r.is_active ? 'active' : 'inactive'})`);
  });
  ...
  if (viewPerm && viewPerm.granted) {
    ...
    console.log('❌ User SHOULD be redirected (Access Denied)');
  }

  console.log('═'.repeat(70));
}

checkBrentPermissions().catch(console.error);
