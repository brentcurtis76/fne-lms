-- =============================================================================
-- 20260813120300_reschedule_availability_guard.sql — Z7 R6
--
-- Additive identical-signature replacement of public.reschedule_session_hours.
-- Historical applied migrations remain immutable. Duration-changing tracked rows
-- now require exactly one canonical get_bucket_summary bucket before the ledger
-- or revision can commit; apply_session_reschedule supplies transaction rollback
-- for the source-session update. No DROP, destructive ALTER, or RLS change.
-- =============================================================================
...
COMMENT ON FUNCTION public.reschedule_session_hours(uuid, uuid) IS
  'Z7 R6 additive identical-signature replacement: atomically reconciles pre-execution schedule changes and refuses before any committed write unless a duration-changing tracked session has exactly one finite, range-bounded, two-decimal, arithmetically coherent get_bucket_summary row. Date-only and genuine no-ledger legacy cases remain supported. Service-role only through apply_session_reschedule.';
