...
VALUES ('00000000-0000-0000-0000-00000000f050', 'fpc-red-creator@fpc.local', '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated')
ON CONFLICT (id) DO NOTHING;

INSERT INTO profiles (id, email, name, approval_status)
VALUES ('00000000-0000-0000-0000-00000000f050', 'fpc-red-creator@fpc.local', 'FPC Red Creator Sintetico', 'approved')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.redes_de_colegios (id, nombre, descripcion, created_by)
VALUES ('00000000-0000-0000-0000-00000000f051', 'Red Sintetica FPC Boundary', 'Red sintetica para pgTAP. No es una red real.', '00000000-0000-0000-0000-00000000f050')
ON CONFLICT (id) DO NOTHING;
...
BEGIN
  ...
  VALUES (v_id, p_role::public.user_role_type, 9601, true,
          CASE WHEN p_role = 'supervisor_de_red'
               THEN '00000000-0000-0000-0000-00000000f051'::uuid
          END);
