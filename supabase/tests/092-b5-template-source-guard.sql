-- PROC-B5 F1: owner-only template source revision bump + root UPDATE guard.
...
-- ---------------------------------------------------------------------------
-- A1: signatures, SECURITY DEFINER, pinned search_path, trigger shape, ACL
-- ---------------------------------------------------------------------------
SELECT is(
  pg_get_function_result(to_regprocedure('public.bump_template_source_revisions(uuid[])')),
  'void',
...
SELECT is(
  pg_get_function_result(to_regprocedure('public.guard_assessment_template_source_revision()')),
  'trigger',
...
-- tgtype 17 = ROW(1) | UPDATE(16); not BEFORE, not INSERT/DELETE/TRUNCATE.
SELECT is(
  (SELECT array_agg(t.tgtype::INT) FROM pg_trigger t
    WHERE t.tgfoid = to_regprocedure('public.guard_assessment_template_source_revision()')
...
-- Real invocations are rejected.
SET LOCAL ROLE anon;
SELECT set_config('request.jwt.claims', '{"role":"anon"}', true);
SELECT throws_ok(
  $$SELECT public.bump_template_source_revisions(ARRAY['b5100000-0000-4000-8000-000000000001'::UUID])$$,
  '42501', NULL,
...
-- ---------------------------------------------------------------------------
-- A2: owner helper semantics (as postgres/owner)
-- ---------------------------------------------------------------------------
SELECT is(
  (SELECT count(*)::INT FROM public.assessment_template_source_revisions
...
SELECT lives_ok(
  $$SELECT public.bump_template_source_revisions(ARRAY[
...
SELECT is(
  ARRAY[pg_temp.b5_rev('b5100000-0000-4000-8000-000000000001'),
...
-- ---------------------------------------------------------------------------
-- A3: root UPDATE trigger (as postgres unless noted)
-- ---------------------------------------------------------------------------
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET name = 'B5 root synthetic'
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET name = 'B5 root synthetic B'
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET description = 'B5 root synthetic description'
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
-- area domain may be constrained; use the first accepted synthetic alternate.
DO $$
DECLARE
  v_candidate TEXT;
BEGIN
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET grade_id = 951001
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET scoring_config = '{"b5": {"weight": 1}}'::JSONB
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET created_at = created_at - INTERVAL '1 day'
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET description = 'B5 root synthetic multi-row'
...
-- API caller without EXECUTE still triggers the owner-run guard.
SET LOCAL ROLE service_role;
SELECT set_config('request.jwt.claims', '{"role":"service_role"}', true);
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET name = 'B5 root synthetic 2 service'
...
-- Mechanical-only fields must not bump.
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET version = '1.0.1'
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
  'A3 mechanical version update succeeds'
);
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET updated_at = now() + INTERVAL '1 hour'
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
  'A3 mechanical updated_at update succeeds'
);
SELECT lives_ok(
  $$UPDATE public.assessment_templates SET created_by = tests.get_supabase_uid('b5root_admin')
     WHERE id = 'b5100000-0000-4000-8000-000000000001'$$,
...
-- ---------------------------------------------------------------------------
-- A4: rollback, overflow, direct counter writes denied
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  v_inflight BIGINT;
...
SELECT is(
  (SELECT name FROM public.assessment_templates WHERE id = 'b5100000-0000-4000-8000-000000000002'),
...
SELECT throws_ok(
  $$UPDATE public.assessment_templates SET name = 'B5 root synthetic overflow'
     WHERE id = 'b5100000-0000-4000-8000-000000000002'$$,
  '22003', NULL,
...
SELECT throws_ok(
  $$UPDATE public.assessment_template_source_revisions SET revision = 99
     WHERE template_id = 'b5100000-0000-4000-8000-000000000001'$$,
  '42501', NULL,
