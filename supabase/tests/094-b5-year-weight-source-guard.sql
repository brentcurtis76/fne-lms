...
SELECT throws_ok($$INSERT INTO public.assessment_entity_year_weights
  (id, template_id, entity_type, entity_id, year, weight)
  VALUES ('b5300000-0000-4000-8004-000000000007',
          'b5300000-0000-4000-8000-000000000001', 'objective',
          'b5300000-0000-4000-8003-000000000007', 1, 1)$$,
  '42501', NULL, 'anon year weight insert is denied');
RESET ROLE;
SELECT is(pg_temp.deltas('denied'), 'none', 'denied role leaves source revisions unchanged');
SELECT is((SELECT count(*) FROM public.assessment_entity_year_weights WHERE id = pg_temp.w(7)),
          0::BIGINT, 'denied role leaves no year weight row');
...
SELECT throws_ok($$UPDATE public.assessment_entity_year_weights
  SET template_id = pg_temp.e(99) WHERE id = pg_temp.w(1)$$,
  '23503', NULL, 'invalid template owner rejects move');
SELECT throws_ok($$INSERT INTO public.assessment_entity_year_weights
  (id, template_id, entity_type, entity_id, year, weight)
  VALUES (pg_temp.w(9), pg_temp.e(99), 'objective', pg_temp.e(9), 1, 1)$$,
  '23503', NULL, 'invalid template owner rejects insert');
SELECT is(pg_temp.deltas('invalid'), 'none', 'invalid owner attempts leave every source revision unchanged');
SELECT is((SELECT template_id FROM public.assessment_entity_year_weights WHERE id = pg_temp.w(1)),
          pg_temp.t('A'), 'invalid move preserves original owner');
SELECT is((SELECT count(*) FROM public.assessment_entity_year_weights WHERE id = pg_temp.w(9)),
          0::BIGINT, 'invalid insert leaves no row');

SELECT pg_temp.mark('null');
SELECT throws_ok($$UPDATE public.assessment_entity_year_weights
  SET weight = NULL WHERE id = pg_temp.w(1)$$,
  '23502', NULL, 'NULL weight is rejected');
SELECT throws_ok($$UPDATE public.assessment_entity_year_weights
  SET template_id = NULL WHERE id = pg_temp.w(1)$$,
  '23502', NULL, 'NULL template owner is rejected');
SELECT is(pg_temp.deltas('null'), 'none', 'rejected NULL updates leave source revisions unchanged');
SELECT is((SELECT weight FROM public.assessment_entity_year_weights WHERE id = pg_temp.w(1)),
          4::numeric, 'rejected NULL update preserves weight');
