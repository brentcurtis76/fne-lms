...
SELECT throws_ok($$INSERT INTO public.assessment_year_expectations
  (id, template_id, indicator_id, generation_type)
  VALUES (pg_temp.x(10), pg_temp.t('A'), pg_temp.i(1), 'GT')$$,
  '42501', NULL, 'anon year expectation insert is denied');
RESET ROLE;
SELECT is(pg_temp.deltas('denied'), 'none', 'denied role leaves source revisions unchanged');
SELECT is((SELECT count(*) FROM public.assessment_year_expectations WHERE id = pg_temp.x(10)),
          0::BIGINT, 'denied role leaves no year expectation row');

SELECT pg_temp.mark('invalid');
SELECT throws_ok($$UPDATE public.assessment_year_expectations
  SET template_id = pg_temp.t('Z') WHERE id = pg_temp.x(1)$$,
  '23503', NULL, 'invalid template owner rejects move');
SELECT throws_ok($$INSERT INTO public.assessment_year_expectations
  (id, template_id, indicator_id, generation_type)
  VALUES (pg_temp.x(10), pg_temp.t('Z'), pg_temp.i(1), 'GT')$$,
  '23503', NULL, 'invalid template owner rejects insert');
SELECT is(pg_temp.deltas('invalid'), 'none', 'invalid owner attempts leave revisions unchanged');
SELECT is((SELECT template_id FROM public.assessment_year_expectations WHERE id = pg_temp.x(1)),
